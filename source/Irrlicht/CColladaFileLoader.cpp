...
void CColladaFileLoader::readEffect(io::IXMLReaderUTF8* reader, SColladaEffect * effect)
{
	...
	bool bilinear = effect->Parameters->getAttributeAsBool("bilinear");
	bool trilinear = effect->Parameters->getAttributeAsBool("trilinear");
	uint32_t anisotropy =  effect->Parameters->getAttributeAsBool("anisotropic");
	for (u32 i=0; i<video::MATERIAL_MAX_TEXTURES; ++i)
	{
		effect->Mat.TextureLayer[i].SamplingParams.TextureWrapU = twu;
		effect->Mat.TextureLayer[i].SamplingParams.TextureWrapV = twv;
		effect->Mat.TextureLayer[i].SamplingParams.MinFilter = bilinear ? (trilinear ? video::ETFT_LINEAR_LINEARMIP:video::ETFT_LINEAR_NEARESTMIP):(trilinear ? video::ETFT_NEAREST_LINEARMIP:video::ETFT_NEAREST_NEARESTMIP);
		effect->Mat.TextureLayer[i].SamplingParams.MaxFilter = bilinear ? video::ETFT_LINEAR_NO_MIP:video::ETFT_NEAREST_NO_MIP;
		effect->Mat.TextureLayer[i].SamplingParams.AnisotropicFilter = anisotropy;
	}
}
