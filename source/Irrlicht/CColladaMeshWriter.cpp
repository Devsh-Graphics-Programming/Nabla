...
irr::core::stringw CColladaMeshWriter::minTexfilterToString(video::E_TEXTURE_FILTER_TYPE etft) const
{
	switch (etft)
    {
    case video::ETFT_NEAREST_NO_MIP:
    case video::ETFT_LINEAR_NO_MIP:
    case video::ETFT_NEAREST_NEARESTMIP:
    case video::ETFT_LINEAR_NEARESTMIP:
    case video::ETFT_NEAREST_LINEARMIP:
    case video::ETFT_LINEAR_LINEARMIP:
        return video::aMinFilterNames[etft];
    default:
        return L"NONE";
	}
}

inline irr::core::stringw CColladaMeshWriter::magTexfilterToString(video::E_TEXTURE_FILTER_TYPE etft) const
{
	switch (etft)
    {
    case video::ETFT_NEAREST_NO_MIP:
    ...
    case video::ETFT_NEAREST_LINEARMIP:
    case video::ETFT_LINEAR_LINEARMIP:
        return video::aMaxFilterNames[etft&0x1u];
    default:
        return L"NONE";
	}
}
...
void CColladaMeshWriter::writeMaterialEffect(const irr::core::stringw& materialfxname, const video::SMaterial & material)
{
	...
	if ( getWriteTextures() )
	{
		...
		//			<wrap_s>WRAP</wrap_s>
					...
					Writer->writeText(toString((video::E_TEXTURE_CLAMP)layer.SamplingParams.TextureWrapU).c_str());
					Writer->writeClosingTag(L"wrap_s");
					Writer->writeLineBreak();

		//			<wrap_t>WRAP</wrap_t>
					Writer->writeElement(L"wrap_t", false);
					Writer->writeText(toString((video::E_TEXTURE_CLAMP)layer.SamplingParams.TextureWrapV).c_str());
					Writer->writeClosingTag(L"wrap_t");
					Writer->writeLineBreak();

		//			<minfilter>LINEAR_MIPMAP_LINEAR</minfilter>
					Writer->writeElement(L"minfilter", false);
					Writer->writeText(minTexfilterToString((video::E_TEXTURE_FILTER_TYPE)layer.SamplingParams.MinFilter).c_str());
					Writer->writeClosingTag(L"minfilter");
					Writer->writeLineBreak();

		//			<magfilter>LINEAR</magfilter>
					Writer->writeElement(L"magfilter", false);
					Writer->writeText(magTexfilterToString((video::E_TEXTURE_FILTER_TYPE)layer.SamplingParams.MaxFilter).c_str());
					...
					if ( layer.SamplingParams.AnisotropicFilter )
					{
...
void CColladaMeshWriter::writeMeshGeometry(const irr::core::stringw& meshname, scene::IMesh* mesh)
{
	...
	for (i=0; i<mesh->getMeshBufferCount(); ++i)
	{
        ...
        {
            os::Printer::log("ColladaMeshWriter 32bit Index Meshbuffers NOT SUPPORTED",ELL_ERROR);
        }

        u16* indexBuffer = (u16*)buffer->getIndices();
		...
		{
			strP = "";
			strP += indexBuffer[(p*3) + 0] + posIdx;
			strP += " ";
			strP += indexBuffer[(p*3) + 0] + tCoordIdx;
			strP += " ";
			strP += indexBuffer[(p*3) + 0] + normalIdx;
			strP += " ";
			if (has2ndTexCoords)
			{
				strP += indexBuffer[(p*3) + 0] + tCoord2Idx;
				strP += " ";
			}

			strP += indexBuffer[(p*3) + 1] + posIdx;
			strP += " ";
			strP += indexBuffer[(p*3) + 1] + tCoordIdx;
			strP += " ";
			strP += indexBuffer[(p*3) + 1] + normalIdx;
			strP += " ";
			if (has2ndTexCoords)
			{
				strP += indexBuffer[(p*3) + 1] + tCoord2Idx;
				strP += " ";
			}

			strP += indexBuffer[(p*3) + 2] + posIdx;
			strP += " ";
			strP += indexBuffer[(p*3) + 2] + tCoordIdx;
			strP += " ";
			strP += indexBuffer[(p*3) + 2] + normalIdx;
			if (has2ndTexCoords)
			{
				strP += " ";
				strP += indexBuffer[(p*3) + 2] + tCoord2Idx;
			}
			strP += " ";
