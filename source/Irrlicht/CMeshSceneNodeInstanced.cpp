...
bool CMeshSceneNodeInstanced::setLoDMeshes(const core::vector<MeshLoD>& levelsOfDetail, const size_t& dataSizePerInstanceOutput, const video::SMaterial& lodSelectionShader, VaoSetupOverrideFunc vaoSetupOverride, const size_t shaderLoDsPerPass, void* overrideUserData, const size_t& extraDataSizePerInstanceInput)
{
    ...
    buffSize += video::CResizableDoubleBufferingAllocator<core::ContiguousPoolAddressAllocatorST<uint32_t>,true>::calcBufferAlignPadding(buffSize,driver,0x1u<<core::findMSB(dataPerInstanceInputSize));
...
uint32_t CMeshSceneNodeInstanced::addInstance(const core::matrix4x3& relativeTransform, void* extraData)
{
    ...
    if (!addInstances(&ix,1,&relativeTransform,extraData))
        return std::remove_pointer<decltype(instanceDataAllocator)>::type::alloc_traits::allocator_type::invalid_address;
...
core::matrix4x3 CMeshSceneNodeInstanced::getInstanceTransform(const uint32_t& instanceID)
{
    ...
    if (redir==std::remove_pointer<decltype(instanceDataAllocator)>::type::alloc_traits::allocator_type::invalid_address)
    {
...
{
    size_t redirect = instanceDataAllocator->getAllocator().get_real_addr(instanceID)+36+48+extraDataInstanceSize;
...
void CMeshSceneNodeInstanced::setInstanceData(const uint32_t& instanceID, void* data)
{
    ...
    size_t redirect = instanceDataAllocator->getAllocator().get_real_addr(instanceID)+36+48;
