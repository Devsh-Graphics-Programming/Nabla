...
u32 CCPUSkinnedMesh::getFrameCount() const
{
...
f32 CCPUSkinnedMesh::getAnimationSpeed() const
{
...
void CCPUSkinnedMesh::setAnimationSpeed(f32 fps)
{
	FramesPerSecond=fps;
}
...
ICPUMesh* CCPUSkinnedMesh::getMesh(s32 frame, s32 detailLevel)
{
	if (frame==-1)
		return this;
...
void CCPUSkinnedMesh::animateMesh(f32 frame, f32 blend)
{
...
void CCPUSkinnedMesh::buildAllLocalAnimatedMatrices()
{
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
		SJoint *joint = AllJoints[i];
...
void CCPUSkinnedMesh::buildAllGlobalAnimatedMatrices(SJoint *joint, SJoint *parentJoint)
{
	if (!joint)
	{
...
void CCPUSkinnedMesh::getFrameData(f32 frame, SJoint *joint,
...
void CCPUSkinnedMesh::skinMesh()
{
	...
	///if (!HardwareSkinning)
	{
		...
		for (i=0; i<AllJoints.size(); ++i)
		{
			...
			{
				SCPUSkinMeshBuffer* Buffer=(*SkinningBuffers)[ AllJoints[i]->AttachedMeshes[j] ];
...
void CCPUSkinnedMesh::skinJoint(SJoint *joint, SJoint *parentJoint)
{
	if (joint->Weights.size())
	{
		...
		core::vectorSIMDf thisVertexMove, thisNormalMove;

		core::array<scene::SCPUSkinMeshBuffer*> &buffersUsed=*SkinningBuffers;
		...
		for (u32 i=0; i<joint->Weights.size(); ++i)
		{
			...
			// Pull this vertex...
			jointVertexPull.transformVect(thisVertexMove.getAsVector3df(), weight.StaticPos.getAsVector3df());

			if (AnimateNormals)
				jointVertexPull.rotateVect(thisNormalMove.getAsVector3df(), weight.StaticNormal.getAsVector3df());

			if (! (*(weight.Moved)) )
			{
				*(weight.Moved) = true;

			    core::vectorSIMDf position = thisVertexMove * weight.strength;
				buffersUsed[weight.buffer_id]->setAttribute(position,buffersUsed[weight.buffer_id]->getPositionAttributeIx(),weight.vertex_id);

				if (AnimateNormals)
                {
                    core::vectorSIMDf normal = thisNormalMove * weight.strength;
                    buffersUsed[weight.buffer_id]->setAttribute(normal,buffersUsed[weight.buffer_id]->getNormalAttributeIx(),weight.vertex_id);
                }
			}
			else
			{
			    core::vectorSIMDf position;
				buffersUsed[weight.buffer_id]->getAttribute(position,buffersUsed[weight.buffer_id]->getPositionAttributeIx(),weight.vertex_id);
				...
				if (AnimateNormals)
                {
                    ...
                    buffersUsed[weight.buffer_id]->getAttribute(normal,buffersUsed[weight.buffer_id]->getNormalAttributeIx(),weight.vertex_id);
...
//! Gets joint count.
u32 CCPUSkinnedMesh::getJointCount() const
{
...
const c8* CCPUSkinnedMesh::getJointName(u32 number) const
{
...
s32 CCPUSkinnedMesh::getJointNumber(const c8* name) const
{
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
...
bool CCPUSkinnedMesh::useAnimationFrom(const ICPUSkinnedMesh *mesh)
{
...
void CCPUSkinnedMesh::updateNormalsWhenAnimating(bool on)
{
	AnimateNormals = on;
}
...
void CCPUSkinnedMesh::setInterpolationMode(E_INTERPOLATION_MODE mode)
{
...
core::array<CCPUSkinnedMesh::SJoint*> &CCPUSkinnedMesh::getAllJoints()
{
	return AllJoints;
}


const core::array<CCPUSkinnedMesh::SJoint*> &CCPUSkinnedMesh::getAllJoints() const
{
	return AllJoints;
}
...
void CCPUSkinnedMesh::calculateGlobalMatrices(SJoint *joint,SJoint *parentJoint)
{
	...
	if (HasAnimation && !PreparedForSkinning)
	{
		...
		for(i=0; i < AllJoints.size(); ++i)
		{
			...
			for (j=0; j<joint->Weights.size(); ++j)
			{
				...
				else if (vertex_id>LocalBuffers[buffer_id]->getIndexMaxBound())
				{
		...
		for (i=0; i<AllJoints.size(); ++i)
		{
			...
			for (j=0; j<joint->Weights.size(); ++j)
			{
				...
				LocalBuffers[buffer_id]->getAttribute(joint->Weights[j].StaticPos,LocalBuffers[buffer_id]->getPositionAttributeIx(),vertex_id);
                LocalBuffers[buffer_id]->getAttribute(joint->Weights[j].StaticNormal,LocalBuffers[buffer_id]->getNormalAttributeIx(),vertex_id);
			}
		}
...
void CCPUSkinnedMesh::finalize()
{
	...
	for (i=0; i<LocalBuffers.size(); ++i)
	{
	    ...
	    if (desc&&AnimateNormals)
        {
            if (desc->getAttribType(LocalBuffers[i]->getNormalAttributeIx())!=ECT_FLOAT)
                os::Printer::log("Skinned Meshes ONLY SUPPORT FLOATING POINT NORMAL ATTRIBUTE!\n",ELL_ERROR);
        }
	...
	for (i=0; i<LocalBuffers.size(); ++i)
	{
		...
		Vertices_Moved[i].set_used(LocalBuffers[i]->getIndexMaxBound()+1);
	}
...
void CCPUSkinnedMesh::updateBoundingBox(void)
{
	if(!SkinningBuffers)
		return;

	core::array<SCPUSkinMeshBuffer*> & buffer = *SkinningBuffers;
	BoundingBox.reset(0,0,0);
...
CCPUSkinnedMesh::SPositionKey *CCPUSkinnedMesh::addPositionKey(SJoint *joint)
{
	if (!joint)
		return 0;
...
CCPUSkinnedMesh::SScaleKey *CCPUSkinnedMesh::addScaleKey(SJoint *joint)
{
	if (!joint)
		return 0;
...
CCPUSkinnedMesh::SRotationKey *CCPUSkinnedMesh::addRotationKey(SJoint *joint)
{
	if (!joint)
		return 0;
...
CCPUSkinnedMesh::SWeight *CCPUSkinnedMesh::addWeight(SJoint *joint)
{
	if (!joint)
		return 0;
...
void CCPUSkinnedMesh::normalizeWeights()
{
	...
	for (i=0; i<LocalBuffers.size(); ++i)
	{
		...
		verticesTotalWeight[i].set_used(LocalBuffers[i]->getIndexMaxBound()+1);
	}
...
void CCPUSkinnedMesh::recoverJointsFromMesh(core::array<IBoneSceneNode*> &jointChildSceneNodes)
{
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
...
void CCPUSkinnedMesh::transferJointsToMesh(const core::array<IBoneSceneNode*> &jointChildSceneNodes)
{
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
...
void CCPUSkinnedMesh::transferOnlyJointsHintsToMesh(const core::array<IBoneSceneNode*> &jointChildSceneNodes)
{
	for (u32 i=0; i<AllJoints.size(); ++i)
	{
		const IBoneSceneNode* const node=jointChildSceneNodes[i];
...
void CCPUSkinnedMesh::addJoints(core::array<IBoneSceneNode*> &jointChildSceneNodes,
