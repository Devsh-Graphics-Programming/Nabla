	...
	: IBillboardSceneNode(parent, mgr, id, position)
{
    ...
    desc->setVertexAttrBuffer(vertexBuffer,asset::EVAI_ATTR0,video::EF_R32G32B32_SFLOAT);
    vertexBuffer->drop();
		...
		const video::SColor& bottomColor)
{
    ...
    desc->setVertexAttrBuffer(buf,asset::EVAI_ATTR1, video::EF_B8G8R8A8_UNORM);
    desc->setVertexAttrBuffer(buf,asset::EVAI_ATTR2,video::EF_R8G8_USCALED,0,16);
    buf->drop();
}
