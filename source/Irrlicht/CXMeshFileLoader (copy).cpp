...
bool CXMeshFileLoader::load(io::IReadFile* file)
{
	...
	for (u32 n=0; n<Meshes.size(); ++n)
	{
		...
		#else
		{
			...
			std::vector< s16 > verticesLinkBuffer;
			verticesLinkBuffer.resize(mesh->Positions.size());
			...
			for (i=0;i<mesh->FaceMaterialIndices.size();++i)
			{
				for (u32 id=i*3+0;id<=i*3+2;++id)
				{
					if ((verticesLinkBuffer[mesh->Indices[id]] != -1) && (verticesLinkBuffer[mesh->Indices[id]] != (s16)mesh->FaceMaterialIndices[i]))
					{
						...
						const u32 tmp = mesh->Positions.size();
						mesh->Positions.push_back(mesh->Positions[ mesh->Indices[id] ]);
						if (mesh->Normals.size())
                            mesh->Normals.push_back(mesh->Normals[ mesh->Indices[id] ]);
						if (mesh->TCoords.size())
                            mesh->TCoords.push_back(mesh->TCoords[ mesh->Indices[id] ]);
						if (mesh->TCoords_2.size())
                            mesh->TCoords_2.push_back(mesh->TCoords_2[ mesh->Indices[id] ]);
						if (mesh->Colors.size())
                            mesh->Colors.push_back(mesh->Colors[ mesh->Indices[id] ]);
						mesh->Indices[id] = tmp;
			...
			if (mesh->FaceMaterialIndices.size() != 0)
			{
				...
				for (i=0; i!=mesh->Buffers.size(); ++i)
                {
                    ...
                    core::ICPUBuffer* buf = new core::ICPUBuffer(vCountArray[i]*4*3);
                    desc->mapVertexAttrBuffer(buf,EVAI_ATTR0,ECPA_THREE,ECT_FLOAT);
                    buf->drop();

                    if (mesh->Colors.size())
                    {
                        buf = new core::ICPUBuffer(vCountArray[i]*4);
                        desc->mapVertexAttrBuffer(buf,EVAI_ATTR1,ECPA_FOUR,ECT_NORMALIZED_UNSIGNED_BYTE);
                        buf->drop();
                    }

                    if (mesh->TCoords.size())
                    {
                        buf = new core::ICPUBuffer(vCountArray[i]*4*2);
                        desc->mapVertexAttrBuffer(buf,EVAI_ATTR2,ECPA_TWO,ECT_FLOAT);
                        buf->drop();
                    }

                    if (mesh->Normals.size())
                    {
                        buf = new core::ICPUBuffer(vCountArray[i]*4*3);
                        desc->mapVertexAttrBuffer(buf,EVAI_ATTR3,ECPA_THREE,ECT_FLOAT);
                        buf->drop();

                        mesh->Buffers[i]->setNormalAttributeIx(EVAI_ATTR3);
                    }

                    if (mesh->TCoords_2.size())
                    {
                        buf = new core::ICPUBuffer(vCountArray[i]*4*2);
                        desc->mapVertexAttrBuffer(buf,EVAI_ATTR4,ECPA_TWO,ECT_FLOAT);
                        buf->drop();
                    }
                }


				verticesLinkIndex.set_used(mesh->Positions.size());
				memset(vCountArray, 0, mesh->Buffers.size()*sizeof(u32));
				...
				for (i=0; i<mesh->Positions.size(); ++i)
				{
                    ...
                    uint32_t oldIx = vCountArray[ verticesLinkBuffer[i] ];
                    verticesLinkIndex[i] = oldIx;
                    ((core::vector3df*)buffer->getMeshDataAndFormat()->getMappedBuffer(EVAI_ATTR0)->getPointer())[oldIx] = mesh->Positions[i];
                    // We have a problem with correct extra attribute handling here
                    // crash fixed for now by checking the values
                    if (i<mesh->Colors.size())
                        ((uint32_t*)buffer->getMeshDataAndFormat()->getMappedBuffer(EVAI_ATTR1)->getPointer())[oldIx] = mesh->Colors[i];
                    if (i<mesh->TCoords.size())
                        ((core::vector2df*)buffer->getMeshDataAndFormat()->getMappedBuffer(EVAI_ATTR2)->getPointer())[oldIx] = mesh->TCoords[i];
                    if (i<mesh->Normals.size())
                        ((core::vector3df*)buffer->getMeshDataAndFormat()->getMappedBuffer(EVAI_ATTR3)->getPointer())[oldIx] = mesh->Normals[i];
                    if (i<mesh->TCoords_2.size())
                        ((core::vector2df*)buffer->getMeshDataAndFormat()->getMappedBuffer(EVAI_ATTR4)->getPointer())[oldIx] = mesh->TCoords_2[i];
				...
				for (i=0; i!=mesh->Buffers.size(); ++i)
                {
                    ...
                    if (buffer->getIndexType()==video::EIT_32BIT)
                        ...
                        buf = new core::ICPUBuffer(vCountArray[i]*2);

					buffer->getMeshDataAndFormat()->mapIndexBuffer(buf);
					buf->drop();

                    buffer->setIndexCount(vCountArray[i]);
                }
				memset(vCountArray, 0, mesh->Buffers.size()*sizeof(u32));
				...
				for (i=0; i<mesh->FaceMaterialIndices.size(); ++i)
				{
                    ...
                    if (buffer->getIndexType()==video::EIT_32BIT)
                    {
                        for (u32 id=i*3+0; id!=i*3+3; ++id)
                            ((uint32_t*)buffer->getIndices())[vCountArray[mesh->FaceMaterialIndices[i]]++] = verticesLinkIndex[ mesh->Indices[id] ];
                    }
                    else
                    {
                        for (u32 id=i*3+0; id!=i*3+3; ++id)
                            ((uint16_t*)buffer->getIndices())[vCountArray[mesh->FaceMaterialIndices[i]]++] = verticesLinkIndex[ mesh->Indices[id] ];
                    }
				}
				delete [] vCountArray;
			}
...
bool CXMeshFileLoader::parseDataObjectFrame(CCPUSkinnedMesh::SJoint *Parent)
{
#ifdef _XREADER_DEBUG
	...
	CCPUSkinnedMesh::SJoint *joint=0;
...
bool CXMeshFileLoader::parseDataObjectMesh(SXMesh* mesh)
{
	...
	// read vertices
	mesh->Positions.resize(nVertices);
	mesh->Colors.reserve(nVertices);
	mesh->TCoords.resize(nVertices);
	mesh->Normals.resize(nVertices);
	for (u32 n=0; n<nVertices; ++n)
	{
		readVector3(mesh->Positions[n]);
	}
	...
	while(true)
	{
		...
		if (objectName == "DeclData")
		{
			...
			mesh->TCoords.resize(mesh->Positions.size());
			if ((uv2pos != -1) && (uv2type == 1))
				mesh->TCoords_2.resize(mesh->Positions.size());
			for (j=0; j<mesh->Positions.size(); ++j)
			{
				if ((normalpos != -1) && (normaltype == 2))
					mesh->Normals[j].set(*((core::vector3df*)(dataptr+normalpos)));
				if ((uvpos != -1) && (uvtype == 1))
					mesh->TCoords[j].set(*((core::vector2df*)(dataptr+uvpos)));
				if ((uv2pos != -1) && (uv2type == 1))
					mesh->TCoords_2[j].set(*((core::vector2df*)(dataptr+uv2pos)));
				dataptr += size;
			}
			delete [] data;
		}
		...
		if (objectName == "FVFData")
		{
			...
			if (dataformat&0x102) // 2nd uv set
			{
				...
				for (u32 j=0; j<mesh->Positions.size(); ++j)
				{
					mesh->TCoords_2[j].set(*((core::vector2df*)(dataptr)));
					dataptr += size;
				}
			}
			delete [] data;
...
bool CXMeshFileLoader::parseDataObjectSkinWeights(SXMesh* mesh)
{
	...
	for (i=0; i<nWeights; ++i)
	{
		...
		CCPUSkinnedMesh::SWeight *weight=AnimatedMesh->addWeight(joint);
	...
	for (u32 k=0; k<nFNormals; ++k)
	{
		...
		if (indexcount == 3)
		{
			...
			for (u32 h=0; h<3; ++h)
			{
				...
				mesh->Normals.resize(core::max_(mesh->Normals.size(),size_t(mesh->Indices[normalidx]+1)));
				mesh->Normals[mesh->Indices[normalidx++]].set(normals[normalnum]);
			}
		}
		else
		{
			...
			for (u32 jk=0; jk<triangles; ++jk)
			{
				mesh->Normals.resize(core::max_(mesh->Normals.size(),size_t(mesh->Indices[normalidx]+3)));
				mesh->Normals[mesh->Indices[normalidx++]].set(normals[polygonfaces[0]]);
				mesh->Normals[mesh->Indices[normalidx++]].set(normals[polygonfaces[jk+1]]);
				mesh->Normals[mesh->Indices[normalidx++]].set(normals[polygonfaces[jk+2]]);
			}
		}
	}
...
bool CXMeshFileLoader::parseDataObjectMeshTextureCoords(SXMesh* mesh)
{
	...
	const size_t nCoords = readInt();
    mesh->TCoords.resize(core::max_(mesh->TCoords.size(),nCoords));
	for (u32 i=0; i<nCoords; ++i)
		readVector2(mesh->TCoords[i]);
...
bool CXMeshFileLoader::parseDataObjectMeshVertexColors(SXMesh* mesh)
{
	...
	for (u32 i=0; i<nColors; ++i)
	{
		...
		video::SColor tmpCol;
		readRGBA(tmpCol);
        mesh->Colors.resize(core::max_(mesh->Colors.size(),Index+1));
...
bool CXMeshFileLoader::parseDataObjectAnimation()
{
	...
	CCPUSkinnedMesh::SJoint animationDump;
