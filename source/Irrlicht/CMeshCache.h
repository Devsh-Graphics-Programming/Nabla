...
class CMeshCache : public IMeshCache<T>
{
	...
	public:
		...
		virtual void addMesh(const io::path& filename, /*IAnimatedMesh*/T* mesh);

		//! Removes a mesh from the cache.
		/** After loading a mesh with getMesh(), the mesh can be removed from the cache
		using this method, freeing a lot of memory. */
		virtual void removeMesh(const /*IAnimatedMesh*/T* const mesh);
		...
		virtual /*IAnimatedMesh*/T* getMeshByName(const io::path& name);
		...
		\param name New name for the mesh.
		\return True if mesh was renamed. */
		virtual bool renameMesh(const /*IAnimatedMesh*/T* const mesh, const io::path& name);
