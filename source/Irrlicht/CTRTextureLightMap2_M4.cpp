...
REALINLINE void CTRTextureLightMap2_M4::scanline_bilinear2_mag ()
{
	...
	const int32_t xStart = irr::core::ceil32( line.x[0] );
	const int32_t xEnd = irr::core::ceil32( line.x[1] ) - 1;
	int32_t dx;
	int32_t i;
	...
	// subTexel
	const float subPixel = ( (float) xStart ) - line.x[0];
	...
	line.w[1] = b;
#else
	const float b = (line.z[1] - line.z[0]) * invDeltaX;
	float a = line.z[0] + ( b * subPixel );
...
#else
			float inversew = FIX_POINT_F32_MUL;
#endif
