...
bool CIrrDeviceSDL::run()
{
	...
	for (uint32_t i=0; i<Joysticks.size(); ++i)
	{
		...
		if (joystick)
		{
			...
			joyevent.JoystickEvent.Axis[SEvent::SJoystickEvent::AXIS_6]=0;
			joyevent.JoystickEvent.Axis[SEvent::SJoystickEvent::AXIS_7]=0;
			for (j=0; j<numAxes; ++j)
...
bool CIrrDeviceSDL::present(video::IImage* surface, void* windowId, core::rect<int32_t>* srcClip)
{
	...
	if ((surface->getColorFormat()==video::ECF_R8G8B8_UINT) ||
			(surface->getColorFormat()==video::ECF_B8G8R8A8_UINT))
	{
...
video::ECOLOR_FORMAT CIrrDeviceSDL::getColorFormat() const
{
	if (Screen)
	{
		...
		{
			if (Screen->format->Amask != 0)
				return video::ECF_B8G8R8A8_UINT;
			else
