...
int32_t CLimitReadFile::read(void* buffer, uint32_t sizeToRead)
{
	...
	int32_t toRead = core::min_<int32_t>(AreaEnd, r + sizeToRead) - core::max_<int32_t>(AreaStart, r);
	if (toRead < 0)
		return 0;
	File->seek(r);
...
{
#if 1
	Pos = core::clamp<int32_t>(finalPos + (relativeMovement ? Pos : 0 ), 0, AreaEnd - AreaStart);
	return true;
#else
	const size_t pos = File->getPos();
