...
void CTRTextureGouraud::setRenderTarget(video::IImage* surface, const core::rect<int32_t>& viewPort)
{
	if (RenderTarget)
		RenderTarget->drop();
...
void CTRTextureGouraud::drawIndexedTriangleList(S2DVertex* vertices, int32_t vertexCount, const uint16_t* indexList, int32_t triangleCount)
{
					...
					while (hSpanBegin < hSpanEnd)
					{
						if (spanZValue > *spanZTarget)
						{
							*spanZTarget = spanZValue;
							uint16_t color = lockedTexture[((spanTy>>8)&textureYMask) * lockedTextureWidth + ((spanTx>>8)&textureXMask)];
