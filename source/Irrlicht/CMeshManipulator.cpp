...
SMesh* CMeshManipulator::createMeshCopy(scene::IMesh* mesh) const
{
	...
	for ( u32 b=0; b<meshBufferCount; ++b)
	{
            ...
            case video::EVT_STANDARD:
                {
                    ...
                    const u16* indices = reinterpret_cast<const u16*>(mb->getIndices());
        ...
        else if (mb->getIndexType()==video::EIT_32BIT)
        {
            ...
            case video::EVT_STANDARD:
                {
                    CMeshBuffer<video::S3DVertex,u32>* buffer = new CMeshBuffer<video::S3DVertex,u32>();
                    buffer->Material = mb->getMaterial();
                    const u32 vcount = mb->getVertexCount();
                    buffer->Vertices.reallocate(vcount);
                    video::S3DVertex* vertices = (video::S3DVertex*)mb->getVertices();
                    ...
                    const u32 icount = mb->getIndexCount();
                    buffer->Indices.reallocate(icount);
                    const u32* indices = reinterpret_cast<const u32*>(mb->getIndices());
                    for (u32 i=0; i < icount; ++i)
                        buffer->Indices.push_back(indices[i]);
                    clone->addMeshBuffer(buffer);
                    buffer->drop();
            ...
            case video::EVT_2TCOORDS:
                {
                    CMeshBuffer<video::S3DVertex2TCoords,u32>* buffer = new CMeshBuffer<video::S3DVertex2TCoords,u32>();
                    buffer->Material = mb->getMaterial();
                    const u32 vcount = mb->getVertexCount();
                    buffer->Vertices.reallocate(vcount);
                    video::S3DVertex2TCoords* vertices = (video::S3DVertex2TCoords*)mb->getVertices();
            ...
            case video::EVT_TANGENTS:
                {
                    CMeshBuffer<video::S3DVertexTangents,u32>* buffer = new CMeshBuffer<video::S3DVertexTangents,u32>();
                    buffer->Material = mb->getMaterial();
                    const u32 vcount = mb->getVertexCount();
                    buffer->Vertices.reallocate(vcount);
                    video::S3DVertexTangents* vertices = (video::S3DVertexTangents*)mb->getVertices();
...
IMesh* CMeshManipulator::createMeshUniquePrimitives(IMesh* mesh) const
{
	...
	for ( u32 b=0; b<meshBufferCount; ++b)
	{
		...
		const u16* idx = reinterpret_cast<const u16*>(mb->getIndices());

        if (mb->getIndexType()!=video::EIT_16BIT)
        {
            clone->drop();
            os::Printer::log("ERROR NON 16BIT INDICES IN MESH BUFFER NOT SUPPORTED",ELL_ERROR);
            return 0;
        }
...
IMesh* CMeshManipulator::createMeshWelded(IMesh *mesh, f32 tolerance) const
{
	...
	for (u32 b=0; b<mesh->getMeshBufferCount(); ++b)
	{
		...
		case video::EVT_STANDARD:
		{
			...
			indices = reinterpret_cast<const u16*>(mb->getIndices());
...
IMesh* CMeshManipulator::createMeshWithTangents(IMesh* mesh, bool recalculateNormals, bool smooth, bool angleWeighted, bool calculateTangents) const
{
	...
	for (u32 b=0; b<meshBufferCount; ++b)
	{
        ...
        if (original->getIndexType()!=video::EIT_16BIT)
        {
            clone->drop();
            os::Printer::log("ERROR NON 16BIT INDICES IN MESH BUFFER NOT SUPPORTED",ELL_ERROR);
            return 0;
		...
		const u16* idx = reinterpret_cast<const u16*>(original->getIndices());
