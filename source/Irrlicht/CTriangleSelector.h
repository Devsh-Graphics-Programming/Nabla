...
public:
	...
	//! Constructs a selector based on a mesh
	CTriangleSelector(const ICPUMesh* mesh, ISceneNode* node);
...
protected:
	//! Create from a mesh
	virtual void createFromMesh(const ICPUMesh* mesh);

	//! Update when the mesh has changed
	virtual void updateFromMesh(const ICPUMesh* mesh) const;
