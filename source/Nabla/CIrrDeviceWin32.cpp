...
struct SJoystickWin32Control
{
    ...
    bool activateJoysticks(core::vector<SJoystickInfo>& joystickInfo)
    {
        ...
        for(uint32_t joystick = 0; joystick < ActiveJoysticks.size(); ++joystick)
        {
            ...
            info.PovHat = (activeJoystick.devcaps.dwPOVs != 0) ? SJoystickInfo::POV_HAT_PRESENT : SJoystickInfo::POV_HAT_ABSENT;
        ...
        for(; joystick < numberOfJoysticks; ++joystick)
        {
            if(JOYERR_NOERROR == joyGetPosEx(joystick, &info) &&
                JOYERR_NOERROR == joyGetDevCaps(joystick, &activeJoystick.Caps, sizeof(activeJoystick.Caps)))
            {
                ...
                returnInfo.PovHat = ((activeJoystick.Caps.wCaps & JOYCAPS_HASPOV) == JOYCAPS_HASPOV) ? SJoystickInfo::POV_HAT_PRESENT : SJoystickInfo::POV_HAT_ABSENT;
...
void CIrrDeviceWin32::resizeIfNecessary()
{
    ...
    else
    {
        ...
        getVideoDriver()->OnResize(nbl::core::dimension2du((uint32_t)r.right, (uint32_t)r.bottom));
        ...
        case VER_PLATFORM_WIN32_NT:
            ...
            if(osvi.dwMajorVersion <= 4)
            {
                sprintf(tmp, "version %ld.%ld %s (Build %ld)",
                    osvi.dwMajorVersion,
                    osvi.dwMinorVersion,
                    nbl::core::stringc(osvi.szCSDVersion).c_str(),
                    osvi.dwBuildNumber & 0xFFFF);
            }
            else
            {
                sprintf(tmp, "%s (Build %ld)", nbl::core::stringc(osvi.szCSDVersion).c_str(),
                    osvi.dwBuildNumber & 0xFFFF);
            }
