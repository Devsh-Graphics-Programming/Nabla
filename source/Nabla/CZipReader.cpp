...
CZipReader::CZipReader(IReadFile* file, bool isGZip)
    : CFileList(file ? file->getFileName() : io::path("")), File(file), IsGZip(isGZip)
{
#ifdef _NBL_DEBUG
...
{
    p = p;
    return _NBL_ALIGNED_MALLOC(size, _NBL_SIMD_ALIGNMENT);
}
void SzFree(void* p, void* address)
{
    p = p;
    _NBL_ALIGNED_FREE(address);
}
