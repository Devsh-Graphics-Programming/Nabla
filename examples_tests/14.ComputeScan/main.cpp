...
class ComputeScan : public ApplicationBase
{
	static constexpr uint32_t BenchmarkingRuns = 1u;

	// Create (an almost) 128MB input buffer
	static constexpr auto in_size = 128u << 20u;
	static constexpr auto in_count = in_size / sizeof(uint32_t) - 23u;

	// Take (an almost) 64MB portion from it to scan
	static constexpr auto begin = in_count / 4 + 110;
	static constexpr auto end = in_count * 3 / 4 - 78;
	static constexpr auto elementCount = end - begin;
...
public:
	...
	uint32_t* cpu_begin;
	uint32_t* in;
	video::CScanner::E_SCAN_TYPE scanType;

	uint32_t currCPUBenchmarkIteration = 0u;
	...
	void onAppInitialized_impl() override
    {
		...
		scanType = video::CScanner::EST_EXCLUSIVE;
		...
		computeQueue = queues[decltype(initOutput)::EQT_COMPUTE];
    ...
    void workLoopBody() override
    {
		...
		currCPUBenchmarkIteration++;
    }
    bool keepRunning() override
    {
		return currCPUBenchmarkIteration < BenchmarkingRuns;
    }
};

NBL_COMMON_API_MAIN(ComputeScan)