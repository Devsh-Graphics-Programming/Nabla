...
{
	static constexpr uint32_t WIN_W = 1024;
...
public:
	...
	core::smart_refctd_ptr<IGPUBuffer> cameraUBO;
	video::created_gpu_object_array<ICPUMesh> gpuMeshes;
	core::smart_refctd_ptr<IGPUDescriptorSet> gpuDS0;
	core::smart_refctd_ptr<IGPUDescriptorSet> gpuDS1;
	core::smart_refctd_ptr<IGPUDescriptorSet> gpuDS2;
	...
	void onAppInitialized_impl() override
	{
		...
		core::smart_refctd_ptr<const ext::MitsubaLoader::CMitsubaMetadata> globalMeta;
		{
			...
			std::string filePath = "mitsuba/bathroom.zip";
...
#define MITSUBA_LOADER_TESTS
			...
			{
				const system::path archPath = sharedInputCWD / filePath;
				core::smart_refctd_ptr<system::IFileArchive> arch = nullptr;
				arch = system->openFileArchive(archPath);

				if (!arch)
					arch = system->openFileArchive(sharedInputCWD / "bathroom.zip");
				assert(arch);
			...
			assert(contents.begin() < contents.end());
	...
	//scene bound
		...
		core::smart_refctd_ptr<asset::ICPUDescriptorSetLayout> ds2Layout;
		{
			...
			bnd[1].stageFlags = asset::IShader::ESS_FRAGMENT;
			bnd[1].type = asset::EDT_COMBINED_IMAGE_SAMPLER;

			bnd[2].binding = 2u;
			bnd[2].count = 1u;
			bnd[2].samplers = nullptr;
			bnd[2].stageFlags = asset::IShader::ESS_FRAGMENT;
			...
			bnd[3].stageFlags = asset::IShader::ESS_FRAGMENT;
		...
		for (auto& mesh : cpuMeshes)
		{
			...
			for (auto meshbuffer : mesh->getMeshBuffers())
			{
				...
				pcr.stageFlags = asset::IShader::ESS_FRAGMENT;
				if (modifiedPipelines.find(pipeline) == modifiedPipelines.end())
				{
					...
					auto* fs = pipeline->getShaderAtStage(asset::ICPUShader::ESS_FRAGMENT);
					auto found = modifiedShaders.find(core::smart_refctd_ptr<asset::ICPUSpecializedShader>(fs));
					if (found != modifiedShaders.end())
						pipeline->setShaderAtStage(asset::ICPUShader::ESS_FRAGMENT, found->second.get());
					else {
						...
						pipeline->setShaderAtStage(asset::ICPUShader::ESS_FRAGMENT, newfs.get());
					}
		...
		gpuMeshes = cpu2gpu.getGPUObjectsFromAssets(cpuMeshes.data(), cpuMeshes.data() + cpuMeshes.size(), cpu2gpuParams);
		cpu2gpuParams.waitForCreationToComplete();

		gpuDS0 = cpu2gpu.getGPUObjectsFromAssets(&cpuDS0.get(), &cpuDS0.get() + 1, cpu2gpuParams)->begin()[0];
		cpu2gpuParams.waitForCreationToComplete();
		...
		IGPUBuffer::SCreationParams cameraUBOCreationParams;
	...
	void workLoopBody() override
	{
		...
		for (uint32_t i = 0u, mbIdx = 0; i < gpuMeshes->size(); i++)
		{
			...
			for (auto mb : mesh->getMeshBuffers())
			{
				...
				commandBuffer->bindDescriptorSets(asset::EPBP_GRAPHICS, gpuRenderpassIndependentPipeline->getLayout(), 0u, 3u, ds, 0u);
				commandBuffer->pushConstants(pipeline->getLayout(), core::bitflag(video::IGPUShader::ESS_VERTEX) | video::IGPUShader::ESS_FRAGMENT, 0u, sizeof(float), mb->getPushConstantsDataPtr());
