...
public:
	...
	void workLoopBody() override
	{
		...
		bool rdocCaptureTriggered = false; // we want to capture the frames when objects get removed from the contiguous GPU ECS
		{
			...
			// Update instances buffer 
			{
				...
				const auto leftoverDWORDs = propertyPoolHandler->transferProperties(
					utilities->getDefaultUpStreamingBuffer(), cb.get(), fence.get(), queues[CommonAPI::InitOutput::EQT_GRAPHICS], scratch,
			...
			// erase, done after update to avoid having a situation where we update stuff we just erased (also erase moves data items around)
			{
				...
				if (instancesToMove)
				{
					...
					queues[CommonAPI::InitOutput::EQT_GRAPHICS]->startCapture();
		...
		CommonAPI::Submit(logicalDevice.get(), swapchain.get(), cb.get(), queues[CommonAPI::InitOutput::EQT_GRAPHICS], semaphoresToWait ? semaphoresToWait[0] : nullptr, m_renderFinished[m_resourceIx].get(), fence.get());
		if (rdocCaptureTriggered)
			queues[CommonAPI::InitOutput::EQT_GRAPHICS]->endCapture();
		CommonAPI::Present(logicalDevice.get(), swapchain.get(), queues[CommonAPI::InitOutput::EQT_GRAPHICS], m_renderFinished[m_resourceIx].get(), imgnum);
	}
