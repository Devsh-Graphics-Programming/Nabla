...
public:
    ...
    void onAppInitialized_impl(void* data) override
    {
        ...
        core::smart_refctd_ptr<asset::ICPUDescriptorSetLayout> cpu_ds2layout;
        {
            ...
            engine->oit.getDSLayoutBindings<asset::ICPUDescriptorSetLayout>(bnd);
        ...
        core::smart_refctd_ptr<video::IGPUDescriptorSetLayout> ds2layout;
        {
            ...
            ds2layout = cpu2gpu.getGPUObjectsFromAssets(&cpu_ds2layout.get(), &cpu_ds2layout.get() + 1, engine->cpu2gpuParams)->operator[](0);
        }

        auto ds2pool = engine->logicalDevice->createDescriptorPoolForDSLayouts(static_cast<video::IDescriptorPool::E_CREATE_FLAGS>(0), &ds2layout.get(), &ds2layout.get() + 1);
        {
            engine->ds2 = engine->logicalDevice->createGPUDescriptorSet(ds2pool.get(), core::smart_refctd_ptr(ds2layout));
            ...
            engine->oit.getDSWrites(w, info, engine->ds2.get());

            engine->logicalDevice->updateDescriptorSets(3u, w, 0u, nullptr);
        }

        {
            auto layout = engine->logicalDevice->createGPUPipelineLayout(nullptr, nullptr, nullptr, nullptr, core::smart_refctd_ptr(ds2layout), nullptr);

            const auto& proto = engine->oit.getResolveProtoPipeline();

            video::IGPUSpecializedShader* shaders[2]{ proto.vs.get(), proto.fs.get() };
            auto rpindep = engine->logicalDevice->createGPURenderpassIndependentPipeline(nullptr, std::move(layout), shaders, shaders + 2, proto.vtx, proto.blend, proto.primAsm, proto.raster);

            video::IGPUGraphicsPipeline::SCreationParams pplnparams;
            pplnparams.renderpass = engine->renderpass;
            pplnparams.renderpassIndependent = rpindep;
            pplnparams.subpassIx = 0u;
            engine->oit_resolve_ppln = engine->logicalDevice->createGPUGraphicsPipeline(nullptr, std::move(pplnparams));
        }
...

        {
            ...
            auto meshes_bundle = engine->assetManager->getAsset((CWDOnStartup / "../../media/white_oak.zip/white_oak.obj").string(), loadParams);
        ...
        {
            for (auto& mb : engine->meshRaw->getMeshBufferVector())
            {
...

        {
            ...
            auto fs_bundle = engine->assetManager->getAsset("../oit_fill_nodes.frag", lparams);
            ...
            core::smart_refctd_ptr<video::IGPURenderpassIndependentPipeline> rpindep;
            {
                ...
                auto gpu_bundle = c2g.getGPUObjectsFromAssets(&srcppln.get(), &srcppln.get() + 1, engine->cpu2gpuParams);
            ...
            pplnparams.renderpassIndependent = rpindep;
            pplnparams.subpassIx = 0u;
            engine->oit_pass1_pipeline = engine->logicalDevice->createGPUGraphicsPipeline(nullptr, std::move(pplnparams));
        }
        ...
        {
            for (size_t i = 0; i < engine->gpu_allMeshes.size(); ++i)
            {
                auto gpuIndependentPipeline = engine->gpu_allMeshes[i]->getPipeline();
                ...
                if (engine->gpuPipelines.find(adress) != engine->gpuPipelines.end())
                    continue;
        ...
        engine->frameDataFilled = false;
        engine->frame_count = 0ull;
        engine->time_sum = 0;
        engine->dtList[NBL_FRAMES_TO_AVERAGE] = {};
    ...
    void workLoopBody(void* data) override
    {
        ...
        if (engine->frameNum == 0u)
        {
            engine->oit.invalidateNodesVisibility(commandBuffer.get());
        }
        ...
        //draw opaque
        for (size_t i = 0; i < engine->gpu_opaqueMeshes.size(); ++i)
        {
            auto gpuMeshBuffer = engine->gpu_opaqueMeshes[i].get();
        ...
        // OIT 1st pass
        for (size_t i = 0; i < engine->gpu_transMeshes.size(); ++i)
        {
            auto gpuMeshBuffer = engine->gpu_transMeshes[i].get();

            const video::IGPURenderpassIndependentPipeline* gpuRenderpassIndependentPipeline = gpuMeshBuffer->getPipeline();
            const video::IGPUDescriptorSet* ds3 = gpuMeshBuffer->getAttachedDescriptorSet();
            const video::IGPUDescriptorSet* gpuds2_ptr = engine->ds2.get();
            const video::IGPUDescriptorSet* gpuds3_ptr = gpuMeshBuffer->getAttachedDescriptorSet();

            commandBuffer->bindGraphicsPipeline(engine->oit_pass1_pipeline.get());
            const video::IGPUDescriptorSet* gpuds1_ptr = engine->gpuds1.get();
            commandBuffer->bindDescriptorSets(asset::EPBP_GRAPHICS, engine->oit_pass1_pipeline->getRenderpassIndependentPipeline()->getLayout(), 1u, 1u, &gpuds1_ptr, nullptr);
            commandBuffer->bindDescriptorSets(asset::EPBP_GRAPHICS, engine->oit_pass1_pipeline->getRenderpassIndependentPipeline()->getLayout(), 2u, 1u, &engine->ds2.get());
            if (gpuds3_ptr)
                commandBuffer->bindDescriptorSets(asset::EPBP_GRAPHICS, engine->oit_pass1_pipeline->getRenderpassIndependentPipeline()->getLayout(), 3u, 1u, &gpuds3_ptr, nullptr);
