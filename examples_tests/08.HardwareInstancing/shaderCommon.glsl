...
bool irr_glsl_workgroupAll(in bool value)
{
	...
	uint partition = _IRR_GLSL_WORKGROUP_VOTE_SHARED_SIZE_NEEDED_>>1u;
	barrier();
	memoryBarrierShared();
#if IRR_GLSL_IS_POT(_IRR_GLSL_WORKGROUP_SIZE_)
	if (gl_LocalInvocationIndex<partition)
#else
	if (gl_LocalInvocationIndex<partition && gl_LocalInvocationIndex+partition<_IRR_GLSL_WORKGROUP_SIZE_)
#endif
		_IRR_GLSL_SCRATCH_SHARED_DEFINED_[gl_LocalInvocationIndex] &= _IRR_GLSL_SCRATCH_SHARED_DEFINED_[gl_LocalInvocationIndex+partition];
...
// TODO: depending on subgroup extensions available this will vary in size (usually divided by the subgroup size lower bound)
#define _IRR_GLSL_WORKGROUP_BALLOT_SHARED_SIZE_NEEDED_  IRR_GLSL_ROUND_UP_POT(_IRR_GLSL_WORKGROUP_SIZE_)
...
void irr_glsl_workgroupBallot(in bool value)
{
	...
	if (gl_LocalInvocationIndex<((_IRR_GLSL_WORKGROUP_SIZE_+31u)>>5u))
		_IRR_GLSL_SCRATCH_SHARED_DEFINED_[gl_LocalInvocationIndex] = 0u;
	barrier();
	memoryBarrierShared();
	atomicOr(_IRR_GLSL_SCRATCH_SHARED_DEFINED_[gl_LocalInvocationIndex>>5u],1u<<(gl_LocalInvocationIndex&31u));
	barrier();
}
...
{
	return (_IRR_GLSL_SCRATCH_SHARED_DEFINED_[index>>5u]&(1u<<(index&31u)))!=0u;
}
...
bool irr_glsl_workgroupBallotBitCount();
bool irr_glsl_workgroupBallotInclusiveBitCount();
bool irr_glsl_workgroupBallotExclusiveBitCount();
bool irr_glsl_workgroupBallotFindLSB();
bool irr_glsl_workgroupBallotFindMSB();
...
// TODO: depending on subgroup extensions available this will vary in size (usually divided by the subgroup size lower bound)
#define _IRR_GLSL_WORKGROUP_ARITHMETIC_SHARED_SIZE_NEEDED_  IRR_GLSL_ROUND_UP_POT(_IRR_GLSL_WORKGROUP_SIZE_)
...
bool irr_glsl_workgroupAdd(in bool val);
