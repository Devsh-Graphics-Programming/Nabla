...
int main()
{
    ...
    desc->setVertexAttrBuffer(	core::smart_refctd_ptr<video::IGPUBuffer>(driver->createGPUBufferOnDedMem(reqs, false),core::dont_grab), 
								asset::EVAI_ATTR0, asset::EF_R32_SFLOAT, 0u, 0u, ATTRIB_DIVISOR); // map whatever buffer just to activate whatever vertex attribute (look below)
    {
        ...
        for (size_t i = 0u; i < 100u; ++i)
        {
            ...
            meshes[i]->setMeshDataAndFormat(core::smart_refctd_ptr(desc)); // apparently glDrawArrays does nothing if no vertex attribs are active
        }
    }
