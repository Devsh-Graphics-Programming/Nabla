...
public:
    ...
    void onAppInitialized_impl() override
    {
		...
		logger = system::logger_opt_smart_ptr(core::make_smart_refctd_ptr<CStdoutLoggerAndroid>(
...
#else
		logger = system::logger_opt_smart_ptr(core::make_smart_refctd_ptr<CColoredStdoutLoggerWin32>(
			core::bitflag(system::ILogger::ELL_DEBUG) | system::ILogger::ELL_ERROR | system::ILogger::ELL_INFO | system::ILogger::ELL_PERFORMANCE | system::ILogger::ELL_WARNING
			));
#endif

		video::COpenGLDebugCallback cb(core::smart_refctd_ptr(logger.get()));
		api = video::COpenGLESConnection::create(core::smart_refctd_ptr<system::ISystem>(system), 0, "android-sample", std::move(cb));

#ifdef _NBL_PLATFORM_ANDROID_
		auto surface = video::CSurfaceGLAndroid::create<video::EAT_OPENGL_ES>(core::smart_refctd_ptr<video::COpenGLESConnection>((video::COpenGLESConnection*)api.get()), core::smart_refctd_ptr<nbl::ui::IWindowAndroid>(static_cast<nbl::ui::CWindowAndroid*>(window.get())));
		((CommonAPI::CTemporaryEventCallback*)window->getEventCallback())->setLogger(logger);
#else
		...
		windowsCreationParams.system = core::smart_refctd_ptr(system);
		...
		window = windowManager->createWindow(std::move(windowsCreationParams));
		auto surface = video::CSurfaceGLWin32::create<video::EAT_OPENGL_ES>(core::smart_refctd_ptr<nbl::video::COpenGLESConnection>((video::COpenGLESConnection*)api.get()), core::smart_refctd_ptr<ui::IWindowWin32>(static_cast<ui::IWindowWin32*>(window.get())));
		...
		dev = gpu->createLogicalDevice(dev_params);

		auto device = dev;
		auto* queue = device->getQueue(0u, 0u);

		const uint32_t win_w = window->getWidth();
		const uint32_t win_h = window->getHeight();
...
public:
    ...
    void workLoopBody() override
    {
		...
		auto* queue = dev->getQueue(0u, 0u);

		auto img_acq_sem = dev->createSemaphore();
		auto render_finished_sem = dev->createSemaphore();
...
#ifdef _NBL_PLATFORM_ANDROID_
NBL_ANDROID_MAIN_FUNC(SampleApp, CommonAPI::CTemporaryEventCallback)
#else
NBL_COMMON_API_MAIN(SampleApp)
#endif