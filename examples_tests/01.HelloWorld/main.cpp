...
int main()
{
	...
	video::IPhysicalDevice* gpu = nullptr;
	for (size_t i = 0ull; i < gpus.size(); ++i) // There will always be a single gpu
	{
		gpu = gpus.begin()[i];
		...
		// Find required queue family indices
		// OpenGL will always have one queue family supporting all operations
		{
			const auto& queueFamilyProperties = gpu->getQueueFamilyProperties();
			for (uint32_t familyIndex = 0u; familyIndex < queueFamilyProperties.size(); ++familyIndex)
			{
	...
	nbl::video::ISwapchain::SCreationParams swapchainParams = {};
	swapchainParams.surface = surface;
	swapchainParams.minImageCount = minSwapchainImageCount;
	swapchainParams.surfaceFormat = surfaceFormat;
	swapchainParams.presentMode = presentMode;
	swapchainParams.width = WIN_W;
	swapchainParams.height = WIN_H;
	swapchainParams.queueFamilyIndexCount = static_cast<uint32_t>(queueFamilyIndices.size());
	swapchainParams.queueFamilyIndices = queueFamilyIndices.data();
	swapchainParams.imageSharingMode = imageSharingMode;
	// swapchainParams.preTransform = preTransform;
	swapchainParams.imageUsage = asset::IImage::EUF_COLOR_ATTACHMENT_BIT;
	swapchainParams.oldSwapchain = nullptr;

	core::smart_refctd_ptr<video::ISwapchain> swapchain = device->createSwapchain(std::move(swapchainParams));
