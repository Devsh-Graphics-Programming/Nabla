...
namespace video
{
	...
	EDT_NULL exists.*/
	class IVideoModeList : public virtual core::IReferenceCounted
	{
	public:

		//! Gets amount of video modes in the list.
		/** \return Returns amount of video modes. */
		virtual int32_t getVideoModeCount() const = 0;
		...
		virtual core::dimension2d<uint32_t> getVideoModeResolution(int32_t modeNumber) const = 0;
		...
		as good as possible. */
		virtual core::dimension2d<uint32_t> getVideoModeResolution(const core::dimension2d<uint32_t>& minSize, const core::dimension2d<uint32_t>& maxSize) const = 0;

		//! Get the pixel depth of a video mode in bits.
		/** \param modeNumber: zero based index of the video mode.
		\return Size of each pixel of the specified video mode in bits. */
		virtual int32_t getVideoModeDepth(int32_t modeNumber) const = 0;

		//! Get current desktop screen resolution.
		/** \return Size of screen in pixels of the current desktop video mode. */
		virtual const core::dimension2d<uint32_t>& getDesktopResolution() const = 0;

		//! Get the pixel depth of a video mode in bits.
		/** \return Size of each pixel of the current desktop video mode in bits. */
		virtual int32_t getDesktopDepth() const = 0;
	};
