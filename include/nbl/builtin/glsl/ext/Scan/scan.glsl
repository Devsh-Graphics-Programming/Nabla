...
uint nbl_glsl_scan_getPaddedData(in uint idx, in uint pad_val);
int nbl_glsl_scan_getPaddedData(in uint idx, in int pad_val);
float nbl_glsl_scan_getPaddedData(in uint idx, in float pad_val);
...
#define NBL_GLSL_SCAN_DEFINE_UPSWEEP(BIN_OP_NAME, TYPE, IDENTITY) void nbl_glsl_scan_upsweep##BIN_OP_NAME(in uint idx, out TYPE val)\
{\
