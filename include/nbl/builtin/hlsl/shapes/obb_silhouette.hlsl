...
struct BinSilhouette
{
   ...
   uint32_t getVertexIndex(uint32_t index) NBL_CONST_MEMBER_FUNC { return (data >> (3u * index)) & 0x7u; }

   uint32_t getVertexCount() NBL_CONST_MEMBER_FUNC { return (data >> 29u) & 0x7u; }

   void rotr(uint32_t shift, uint32_t size) { data = nbl::hlsl::rotr(data, shift, size); }

   void rotl(uint32_t shift, uint32_t size) { data = nbl::hlsl::rotl(data, shift, size); }
...
struct ClippedSilhouette
{
   ...
   static ClippedSilhouette create(NBL_CONST_REF_ARG(shapes::OBBView<float32_t>) view)
   {
      ...
      uint32_t rotateAmount = nbl::hlsl::select(wrapAround,
...
// Spherical-law-of-cosines (cos,sin) of the polygon interior angle at vertex `cur`,
// given its CCW neighbors `prev` and `next` (all unit vectors). Each call is
// 3 dots + 4 sqrts + 1 div; no transcendentals. Fed into sincos_accumulator
// so the entire polygon sum collapses to a single acos at the end.
inline void _silAddInteriorAngle(NBL_REF_ARG(math::sincos_accumulator<float32_t>) acc, float32_t3 prev, float32_t3 cur, float32_t3 next)
{
   const float32_t cL = hlsl::dot(prev, cur);
   const float32_t cR = hlsl::dot(cur, next);
   const float32_t cO = hlsl::dot(prev, next);
   const float32_t sL = hlsl::sqrt(hlsl::max(1.0f - cL * cL, 0.0f));
   const float32_t sR = hlsl::sqrt(hlsl::max(1.0f - cR * cR, 0.0f));
   const float32_t r  = 1.0f / hlsl::max(sL * sR, 1e-30f);
   const float32_t cA = hlsl::clamp((cO - cL * cR) * r, -1.0f, 1.0f);
   const float32_t sA = hlsl::sqrt(hlsl::max(1.0f - cA * cA, 0.0f));
   acc.addAngle(cA, sA);
}

// Spherical polygon area = NEE-technique solid angle from silhouette.shadingPoint.
// Girard: area = sum(interior angles) - (n - 2) * pi. We sum interior angles via
// math::sincos_accumulator (one transcendental total) and subtract the constant
// offset analytically. Cascade-on-count keeps each verts[K] read at a literal slot.
inline float32_t silhouetteSolidAngle(NBL_CONST_REF_ARG(ClippedSilhouette) silhouette, NBL_CONST_REF_ARG(shapes::OBBView<float32_t>) view)
{
   const uint32_t n = silhouette.count;
   if (n < 3u)
      return 0.0f;

   float32_t3 v[MaxOBBSilhouetteVertices];
   silhouette.materializeNormalized(view, v);

   using accum_t = math::sincos_accumulator<float32_t>;
   accum_t acc;
   acc.runningSum = complex_t<float32_t>::create(1.0f, 0.0f);
   acc.wraparound = 0.0f;

   if (n == 3u)
   {
      _silAddInteriorAngle(acc, v[2], v[0], v[1]);
      _silAddInteriorAngle(acc, v[0], v[1], v[2]);
      _silAddInteriorAngle(acc, v[1], v[2], v[0]);
   }
   else if (n == 4u)
   {
      _silAddInteriorAngle(acc, v[3], v[0], v[1]);
      _silAddInteriorAngle(acc, v[0], v[1], v[2]);
      _silAddInteriorAngle(acc, v[1], v[2], v[3]);
      _silAddInteriorAngle(acc, v[2], v[3], v[0]);
   }
   else if (n == 5u)
   {
      _silAddInteriorAngle(acc, v[4], v[0], v[1]);
      _silAddInteriorAngle(acc, v[0], v[1], v[2]);
      _silAddInteriorAngle(acc, v[1], v[2], v[3]);
      _silAddInteriorAngle(acc, v[2], v[3], v[4]);
      _silAddInteriorAngle(acc, v[3], v[4], v[0]);
   }
   else if (n == 6u)
   {
      _silAddInteriorAngle(acc, v[5], v[0], v[1]);
      _silAddInteriorAngle(acc, v[0], v[1], v[2]);
      _silAddInteriorAngle(acc, v[1], v[2], v[3]);
      _silAddInteriorAngle(acc, v[2], v[3], v[4]);
      _silAddInteriorAngle(acc, v[3], v[4], v[5]);
      _silAddInteriorAngle(acc, v[4], v[5], v[0]);
   }
   else // n == 7
   {
      _silAddInteriorAngle(acc, v[6], v[0], v[1]);
      _silAddInteriorAngle(acc, v[0], v[1], v[2]);
      _silAddInteriorAngle(acc, v[1], v[2], v[3]);
      _silAddInteriorAngle(acc, v[2], v[3], v[4]);
      _silAddInteriorAngle(acc, v[3], v[4], v[5]);
      _silAddInteriorAngle(acc, v[4], v[5], v[6]);
      _silAddInteriorAngle(acc, v[5], v[6], v[0]);
   }

   // getClampedSumOfArccosMinusPi returns sum - pi; we need sum - (n-2)*pi.
   const float32_t area = acc.getClampedSumOfArccosMinusPi() - float32_t(n - 3u) * numbers::pi<float32_t>;
   return hlsl::max(area, 0.0f);
}

// Cheap NEE-pdf evaluator: returns 1/solidAngle if `dir` lies inside the
// silhouette polygon as seen from silhouette.shadingPoint, 0 otherwise.
// For the BSDF-side MIS path; scores the NEE technique's pdf for a BSDF-sampled
// direction without constructing a full SphericalPyramid (skips caliper search,
// axis selection, inner-sampler creation).
//
// `dir` is a unit world-space direction from silhouette.shadingPoint toward the
// queried point.
inline float32_t silhouettePdfAt(NBL_CONST_REF_ARG(ClippedSilhouette) silhouette, NBL_CONST_REF_ARG(shapes::OBBView<float32_t>) view, float32_t3 dir)
{
   if (silhouette.count < 3u)
      return 0.0f;

   float32_t3 verts[MaxOBBSilhouetteVertices];
   silhouette.materializeNormalized(view, verts);

   SilEdgeNormals en = SilEdgeNormals::create(verts, silhouette.count);
   if (!en.isInside(dir))
      return 0.0f;

   // Reuse the unit-vert array for the solid-angle sum. (Inline rather than
   // calling silhouetteSolidAngle to avoid re-materializing.)
   using accum_t = math::sincos_accumulator<float32_t>;
   accum_t acc;
   acc.runningSum = complex_t<float32_t>::create(1.0f, 0.0f);
   acc.wraparound = 0.0f;
   ...
   if (n == 3u)
   {
      _silAddInteriorAngle(acc, verts[2], verts[0], verts[1]);
      _silAddInteriorAngle(acc, verts[0], verts[1], verts[2]);
      _silAddInteriorAngle(acc, verts[1], verts[2], verts[0]);
   }
   else if (n == 4u)
   {
      _silAddInteriorAngle(acc, verts[3], verts[0], verts[1]);
      _silAddInteriorAngle(acc, verts[0], verts[1], verts[2]);
      _silAddInteriorAngle(acc, verts[1], verts[2], verts[3]);
      _silAddInteriorAngle(acc, verts[2], verts[3], verts[0]);
   }
   else if (n == 5u)
   {
      _silAddInteriorAngle(acc, verts[4], verts[0], verts[1]);
      _silAddInteriorAngle(acc, verts[0], verts[1], verts[2]);
      _silAddInteriorAngle(acc, verts[1], verts[2], verts[3]);
      _silAddInteriorAngle(acc, verts[2], verts[3], verts[4]);
      _silAddInteriorAngle(acc, verts[3], verts[4], verts[0]);
   }
   else if (n == 6u)
   {
      _silAddInteriorAngle(acc, verts[5], verts[0], verts[1]);
      _silAddInteriorAngle(acc, verts[0], verts[1], verts[2]);
      _silAddInteriorAngle(acc, verts[1], verts[2], verts[3]);
      _silAddInteriorAngle(acc, verts[2], verts[3], verts[4]);
      _silAddInteriorAngle(acc, verts[3], verts[4], verts[5]);
      _silAddInteriorAngle(acc, verts[4], verts[5], verts[0]);
   }
   else // n == 7
   {
      _silAddInteriorAngle(acc, verts[6], verts[0], verts[1]);
      _silAddInteriorAngle(acc, verts[0], verts[1], verts[2]);
      _silAddInteriorAngle(acc, verts[1], verts[2], verts[3]);
      _silAddInteriorAngle(acc, verts[2], verts[3], verts[4]);
      _silAddInteriorAngle(acc, verts[3], verts[4], verts[5]);
      _silAddInteriorAngle(acc, verts[4], verts[5], verts[6]);
      _silAddInteriorAngle(acc, verts[5], verts[6], verts[0]);
   }
   const float32_t area = acc.getClampedSumOfArccosMinusPi() - float32_t(n - 3u) * numbers::pi<float32_t>;
