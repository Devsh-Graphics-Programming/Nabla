...
struct SphericalTriangle
{
    ...
    static SphericalTriangle<T> createFromUnitSphereVertices(const vector3_type normalizedVertices[3])
    {
        ...
        retval.cos_vertices = hlsl::clamp((retval.cos_sides - retval.cos_sides.yzx * retval.cos_sides.zxy) * retval.csc_sides.yzx * retval.csc_sides.zxy, hlsl::promote<vector3_type>(-1.0), hlsl::promote<vector3_type>(1.0)); // using Spherical Law of Cosines (TODO: do we need to clamp anymore? since the pyramid angles method introduction?)
        ...
        retval.solid_angle = angle_adder.getSumofArccos() - numbers::pi<scalar_type>;
