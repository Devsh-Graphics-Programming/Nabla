...
struct ceil_div : type_traits::integral_constant<uint32_t, ((N-1) / K) + 1u> {};
...
struct trunc_geom_series<N,K,W,false> : type_traits::integral_constant<uint32_t,ceil_div<N,W>::value+trunc_geom_series<N,K,W*K>::value> {};

template<uint32_t N, uint32_t K, uint32_t W>
struct trunc_geom_series<N,K,W,true> : type_traits::integral_constant<uint32_t,0> {};
}
}
}
}
