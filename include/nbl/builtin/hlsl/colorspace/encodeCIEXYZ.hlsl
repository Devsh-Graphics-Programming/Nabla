...
const float3x3 scRGBtoXYZ = float3x3(
    float3(0.4124564, 0.3575761, 0.1804375),
    float3(0.2126729, 0.7151522, 0.0721750),
    float3(0.0193339, 0.1191920, 0.9503041)
);

const float3x3 sRGBtoXYZ = scRGBtoXYZ;

const float3x3 BT709toXYZ = scRGBtoXYZ;


const float3x3 Display_P3toXYZ = float3x3(
...
const float3x3 DCI_P3toXYZ = float3x3(
    float3(1.0, 0.0, 0.0),
    float3(0.0, 1.0, 0.0),
    float3(0.0, 0.0, 1.0)
);
...
const float3x3 BT2020toXYZ = float3x3(
    float3(0.6369580, 0.1446169, 0.1688810),
    float3(0.2627002, 0.6779981, 0.0593017),
    float3(0.0000000, 0.0280727, 1.0609851)
);

const float3x3 HDR10_ST2084toXYZ = BT2020toXYZ;

const float3x3 DOLBYIVISIONtoXYZ = BT2020toXYZ;

const float3x3 HDR10_HLGtoXYZ = BT2020toXYZ;


const float3x3 AdobeRGBtoXYZ = float3x3(
...
const float3x3 ACES2065_1toXYZ = float3x3(
