...
struct SMicrofacetNormals<Config, BRDF, ndf::PNS_SCHUSSLER, 2 NBL_PARTIAL_REQ_BOT(config_concepts::BasicConfiguration<Config>) >
{
    ...
    NBL_CONSTEXPR_STATIC_INLINE bool IsBSDF = false;
    ...
    value_weight_type evalAndWeight(NBL_CONST_REF_ARG(sample_type) _sample, NBL_CONST_REF_ARG(anisotropic_interaction_type) interaction) NBL_CONST_MEMBER_FUNC
    {
        ...
        const scalar_type clampedNdotNp = hlsl::max(scalar_type(0.0), NdotNp);
        ...
        const scalar_type shadowing = shadowing_method_type::G1(hlsl::max(scalar_type(0.0), NdotL), clampedNdotNp,
        ...
        if (NtdotL > scalar_type(0.0))
        {
            ...
            const scalar_type notShadowedNpMirror = scalar_type(1.0) - shadowing_method_type::G1(hlsl::max(scalar_type(0.0), hlsl::dot(L_reflected.getDirection(), shadingNormal)), clampedNdotNp,
                sample_double_p.getNdotL(BxDFClampMode::BCM_MAX), hlsl::max(scalar_type(0.0), hlsl::dot(L_reflected.getDirection(), Nt)));
            const scalar_type shadowing_t = shadowing_method_type::G1(hlsl::max(scalar_type(0.0), NdotL), hlsl::max(scalar_type(0.0), hlsl::dot(shadingNormal, Nt)),
                                        NpdotL, hlsl::max(scalar_type(0.0), NtdotL));
    ...
    sample_type generate(NBL_CONST_REF_ARG(anisotropic_interaction_type) interaction, const random_type u, NBL_REF_ARG(anisocache_type) _cache) NBL_CONST_MEMBER_FUNC
    {
        ...
        if (u.x < shadowing_method_type::lambdaP(NdotNp, hlsl::max(scalar_type(0.0), NpdotV), hlsl::max(scalar_type(0.0), NtdotV)))
        {
            ...
            _cache.sampleFromNt = false;

            if (!s.isValid())
                return sample_type::createInvalid();

            const vector3_type L = s.getL().getDirection();
            const scalar_type shadowed = shadowing_method_type::G1(hlsl::max(scalar_type(0.0), hlsl::dot(shadingNormal, L)), clampedNdotNp,
        ...
        {
            // do one reflection if we start at Nt
    ...
    scalar_type forwardPdf(NBL_CONST_REF_ARG(sample_type) _sample, NBL_CONST_REF_ARG(anisotropic_interaction_type) interaction, NBL_CONST_REF_ARG(anisocache_type) _cache) NBL_CONST_MEMBER_FUNC
    {
        ...
        if (lambda_p > scalar_type(0.0))
        {
            ...
            const scalar_type NpdotL = _sample.getNdotL(BxDFClampMode::BCM_MAX);
            const scalar_type NtdotL = hlsl::dot(Nt, L);
            pdf += lambda_p * nested_brdf.forwardPdf(_sample, interaction, __createChildCache(_sample, interaction)) * shadowing_method_type::G1(hlsl::max(scalar_type(0.0), NdotL), clampedNdotNp,
                NpdotL, hlsl::max(scalar_type(0.0), NtdotL));
            ...
            if (NtdotL > numeric_limits<scalar_type>::min)
            {
                ...
                pdf += lambda_p * nested_brdf.forwardPdf(sample_reflected, interaction, __createChildCache(sample_reflected, interaction)) * 
                        (scalar_type(1.0) - shadowing_method_type::G1(hlsl::max(scalar_type(0.0), hlsl::dot(shadingNormal, L_reflected.getDirection())), clampedNdotNp, sample_reflected.getNdotL(BxDFClampMode::BCM_MAX), hlsl::max(scalar_type(0.0), NtdotL)));
            }
        }
    ...
    quotient_weight_type quotientAndWeight(NBL_CONST_REF_ARG(sample_type) _sample, NBL_CONST_REF_ARG(anisotropic_interaction_type) interaction, NBL_CONST_REF_ARG(anisocache_type) _cache) NBL_CONST_MEMBER_FUNC
    {
        ...
        if (_cache.sampleIsShadowed)
        {
            const vector3_type L = _sample.getL().getDirection();
            const scalar_type NpdotL = _sample.getNdotL(BxDFClampMode::BCM_MAX);
            quo *= shadowing_method_type::G1(hlsl::max(scalar_type(0.0), hlsl::dot(shadingNormal, L)), hlsl::max(scalar_type(0.0), NdotNp), NpdotL, hlsl::max(scalar_type(0.0), hlsl::dot(Nt, L)));
        }
...
struct SMicrofacetNormals<Config, BRDF, P, 1 NBL_PARTIAL_REQ_BOT(config_concepts::BasicConfiguration<Config>) >
{
    ...
    NBL_CONSTEXPR_STATIC_INLINE bool IsBSDF = false;    // TODO: could probably merge with transmitted 1st order
    using bxdf_type = BRDF;
    using random_type = conditional_t<IsBSDF, vector3_type, vector2_type>;
    struct Cache
    {
    ...
    value_weight_type evalAndWeight(NBL_CONST_REF_ARG(sample_type) _sample, NBL_CONST_REF_ARG(anisotropic_interaction_type) interaction) NBL_CONST_MEMBER_FUNC
    {
        ...
        // i -> p -> o
        {
            value_weight_type eval_p = nested_brdf.evalAndWeight(_sample, interaction);
    ...
    sample_type generate(NBL_CONST_REF_ARG(anisotropic_interaction_type) interaction, const random_type u, NBL_REF_ARG(anisocache_type) _cache) NBL_CONST_MEMBER_FUNC
    {
        ...
        else
        {
            ...
            iso_t.luminosityContributionHint = interaction.getLuminosityContributionHint();
            typename bxdf_type::anisotropic_interaction_type interaction_t = bxdf_type::anisotropic_interaction_type::create(iso_t);
            s = nested_brdf.generate(interaction_t, u, _cache.aniso_cache);
            _cache.sampleFromNt = true;
            _cache.sampleIsShadowed = true;
        }
        if (!s.isValid() || hlsl::dot(shadingNormal, s.getL().getDirection()) < scalar_type(0.0))
            return sample_type::createInvalid();
        return s;
    }
    sample_type generate(NBL_CONST_REF_ARG(isotropic_interaction_type) interaction, const random_type u, NBL_REF_ARG(isocache_type) _cache) NBL_CONST_MEMBER_FUNC
    ...
    scalar_type forwardPdf(NBL_CONST_REF_ARG(sample_type) _sample, NBL_CONST_REF_ARG(anisotropic_interaction_type) interaction, NBL_CONST_REF_ARG(anisocache_type) _cache) NBL_CONST_MEMBER_FUNC
    {
        ...
        if (lambda_p < scalar_type(1.0) && NtdotV > numeric_limits<scalar_type>::min)
        {
            ...
            pdf += (scalar_type(1.0) - lambda_p) * nested_brdf.forwardPdf(sample_t, interaction_t, __createChildCache(sample_t, interaction_t));
        }
