...
public:
        ...
        uint32_t set = DefaultSetNum, uint32_t colorBnd = DefaultColorImgBinding, uint32_t depthBnd = DefaultDepthImgBinding, uint32_t visBnd = DefaultVisImgBinding, uint32_t spinlockBnd = DefaultSpinlockImgBinding)
    {
        ...
        std::tie(
            ...
            m_proto_pipeline.raster) = ext::FullScreenTriangle::createProtoPipeline(c2gparams);
    ...
    uint32_t getDSLayoutBindings(typename DSLType::SBinding* _out_bindings,
        uint32_t _colorBinding = DefaultColorImgBinding, uint32_t _depthBinding = DefaultDepthImgBinding, uint32_t _visBinding = DefaultVisImgBinding, uint32_t _spinlockBinding = DefaultSpinlockImgBinding) const
    {
