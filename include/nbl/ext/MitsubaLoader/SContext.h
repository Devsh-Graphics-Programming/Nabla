...
#ifndef _NBL_EXT_MITSUBA_LOADER_S_CONTEXT_H_INCLUDED_
#define _NBL_EXT_MITSUBA_LOADER_S_CONTEXT_H_INCLUDED_
...
struct SContext
{
	...
	static inline constexpr uint32_t VT_PAGE_SZ_LOG2 = 7u;//128
	static inline constexpr uint32_t VT_PHYSICAL_PAGE_TEX_TILES_PER_DIM_LOG2 = 4u;//16
	static inline constexpr uint32_t VT_PAGE_PADDING = 8u;
	static inline constexpr uint32_t VT_MAX_ALLOCATABLE_TEX_SZ_LOG2 = 12u;//4096
	...
	//
	static std::string imageViewCacheKey(const CElementTexture::Bitmap& bitmap, const CMaterialCompilerFrontend::E_IMAGE_VIEW_SEMANTIC semantic)
	{
	...
	using bsdf_type = const CMaterialCompilerFrontend::front_and_back_t;
	//caches instr buffer instr-wise offset (.first) and instruction count (.second) for each bsdf node
	core::unordered_map<const CElementBSDF*,bsdf_type> instrStreamCache; // TODO: remove
	...
	//material compiler
	CMaterialCompilerFrontend::HashCons frontend_ctx;
	core::smart_refctd_ptr<asset::material_compiler::IR> ir;
	asset::material_compiler::CGLSLRasterBackend::SContext backend_ctx;
	asset::material_compiler::CGLSLRasterBackend backend;
