...
public:
    ...
    {
        AllDiffuse()
            : reflectance(0.5f), alpha(0.2f), useFastApprox(false) {}
        ~AllDiffuse() {}
    ...
    struct RoughSpecularBase
    {
        ...
        RoughSpecularBase(float defaultAlpha)
            : distribution(BECKMANN), specularReflectance(1.f)
        {
            alpha = defaultAlpha;
        }
    ...
    struct TransmissiveBase
    {
        ...
        TransmissiveBase(float _intIOR, float _extIOR)
            : intIOR(_intIOR), extIOR(_extIOR), specularTransmittance(1.f) {}
        TransmissiveBase(const std::string& _intIOR, const std::string& _extIOR)
            : TransmissiveBase(findIOR(_intIOR), findIOR(_extIOR)) {}
    ...
    {
        AllDielectric()
            : RoughSpecularBase(0.1f), TransmissiveBase("bk7", "air") {}
        AllDielectric(float intIOR, float extIOR)
            : RoughSpecularBase(0.1f), TransmissiveBase(intIOR, extIOR) {}
        AllDielectric(const std::string& intIOR, const std::string& extIOR)
            : RoughSpecularBase(0.1f), TransmissiveBase(intIOR, extIOR) {}
    ...
    struct AllConductor : RoughSpecularBase
    {
        ...
        AllConductor(SPropertyElementData&& _eta, SPropertyElementData&& _k)
            : RoughSpecularBase(0.1f), eta(_eta), k(_k), extEta(TransmissiveBase::findIOR("air")) {}
    ...
    {
        AllPlastic()
            : RoughSpecularBase(0.1f), TransmissiveBase("polypropylene", "air"), nonlinear(false) {}
        AllPlastic(float intIOR, float extIOR)
            : RoughSpecularBase(0.1f), TransmissiveBase(intIOR, extIOR), nonlinear(false) {}
        AllPlastic(const std::string& intIOR, const std::string& extIOR)
            : RoughSpecularBase(0.1f), TransmissiveBase(intIOR, extIOR), nonlinear(false) {}
    ...
    struct AllCoating : MetaBSDF, RoughSpecularBase, TransmissiveBase
    {
        ...
        AllCoating()
            : MetaBSDF(), RoughSpecularBase(0.1f), TransmissiveBase("bk7", "air"), thickness(1.f), sigmaA(0.f) {}
    ...
    CElementBSDF(const char* id)
        : IElement(id), type(Type::INVALID)
    {
    }
