...
public:
    ...
    struct alignas(16) LoDTableInfo
    {
        LoDTableInfo()
            : levelCount(0u) {}
        LoDTableInfo(const uint32_t lodLevelCount)
            : levelCount(lodLevelCount)
        {
            std::fill_n(aabbMin, 3u, FLT_MAX);
            std::fill_n(aabbMax, 3u, -FLT_MAX);
        }
        LoDTableInfo(const uint32_t lodLevelCount, const core::aabbox3df& aabb)
            : levelCount(lodLevelCount)
        {
    ...
    {
        LoDInfoBase()
            : drawcallInfoCount(0u), totalDrawcallBoneCount(0u) {}
        LoDInfoBase(const uint16_t drawcallCount)
            : drawcallInfoCount(drawcallCount), totalDrawcallBoneCount(0u) {}
    ...
    public:
        DrawcallInfo()
            : aabb(), drawcallDWORDOffset(video::IDrawIndirectAllocator::invalid_draw_range_begin) {}
