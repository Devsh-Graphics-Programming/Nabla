...
#			endif
		...
		)
		{
			...
			auto getShader = [device](auto uniqueString) -> shader_source_and_path
			{
				...
				return {core::make_smart_refctd_ptr<asset::ICPUShader>(std::move(glsl), asset::IShader::ESS_COMPUTE, asset::IShader::ECT_GLSL, decltype(uniqueString)::value), decltype(uniqueString)::value};
			};
