...
public:
    rational()
        : rational(NumeratorType(0u)) {}
    rational(non_atomic_numerator units)
        : rational(units, 1u) {}
    rational(non_atomic_numerator _numerator, non_atomic_denominator _denominator)
    ...
    template<typename OtherNumType, typename OtherDenType>
    rational(const rational<OtherNumType, OtherDenType>& other)
...
template<typename NumeratorType = int32_t, typename DenominatorType = typename std::make_unsigned<NumeratorType>::type>
rational<NumeratorType, DenominatorType> operator*(const NumeratorType& val, rational<NumeratorType, DenominatorType> const& _this)
{
    return _this * val;
}
