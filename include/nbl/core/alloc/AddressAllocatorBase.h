...
#define DUMMY_DEFAULT_CONSTRUCTOR                                                                                  \
    AddressAllocatorBase() : reservedSpace(nullptr), addressOffset(invalid_address), alignOffset(invalid_address), \
...
protected:
    ...
    inline bool checkResize(_size_type newBuffSz, _size_type newAlignOffset)
    {
        ...
        if(newBuffSz < safe_shrink_size(0u, maxRequestableAlignment))
            return false;
        // need enough space for the new alignment
        if(newAlignOffset > newBuffSz)
            return false;
