...
private:
    ...
    template<class U>
    using cstexpr_supportsArbitraryOrderFrees = decltype(std::declval<U&>().supportsArbitraryOrderFrees);
    template<class U>
    using cstexpr_maxMultiOps = decltype(std::declval<U&>().maxMultiOps);

    template<class U>
    using func_multi_alloc_addr = decltype(std::declval<U&>().multi_alloc_addr(0u, nullptr, nullptr, nullptr, nullptr));
    template<class U>
    using func_multi_free_addr = decltype(std::declval<U&>().multi_free_addr(0u, nullptr, nullptr));

    template<class U>
    using func_get_real_addr = decltype(std::declval<U&>().get_real_addr(0u));
    /// C++17 protected:
public:
    template<class, class = void>
    struct resolve_supportsArbitraryOrderFrees : std::true_type
    {
    };
    template<class, class = void>
    struct resolve_maxMultiOps : std::integral_constant<uint32_t, 256u>
    {
    };

    template<class, class = void>
    struct has_func_multi_alloc_addr : std::false_type
    {
    };
    template<class, class = void>
    struct has_func_multi_free_addr : std::false_type
    {
    };

    template<class, class = void>
    struct has_func_get_real_addr : std::false_type
    {
    };

    template<class U>
    struct resolve_supportsArbitraryOrderFrees<U, std::void_t<cstexpr_supportsArbitraryOrderFrees<U> > >
    ...
    template<class U>
    struct resolve_maxMultiOps<U, std::void_t<cstexpr_maxMultiOps<U> > >
    ...
    template<class U>
    struct has_func_multi_alloc_addr<U, std::void_t<func_multi_alloc_addr<U> > >
        : std::is_same<func_multi_alloc_addr<U>, void>
    {
    };
    template<class U>
    struct has_func_multi_free_addr<U, std::void_t<func_multi_free_addr<U> > >
        : std::is_same<func_multi_free_addr<U>, void>
    {
    };

    template<class U>
    struct has_func_get_real_addr<U, std::void_t<func_get_real_addr<U> > > : std::is_same<func_get_real_addr<U>, size_type>
    {
    };
