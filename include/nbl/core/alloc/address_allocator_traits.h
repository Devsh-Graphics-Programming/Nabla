...
#ifndef _NBL_CORE_ADDRESS_ALLOCATOR_TYPE_TRAITS_H_INCLUDED_
#define _NBL_CORE_ADDRESS_ALLOCATOR_TYPE_TRAITS_H_INCLUDED_
...
// TODO: replace with a template variable and alias
...
struct address_type_traits<uint32_t>
{
    ...
    constexpr static inline uint32_t   invalid_address = 0xdeadbeefu;
};
...
{
    address_type_traits() = delete;
    constexpr static inline uint64_t   invalid_address = 0xdeadbeefBADC0FFEull;
};
...
class address_allocator_traits : protected AddressAlloc //maybe private?
{
    ...
    /// C++17 protected:
    public:
        // Every address allocator inherits from `AddressAllocatorBase` which has a constructor:
        // `AddressAllocatorBase(void* reservedSpc, _size_type addressOffsetToApply, _size_type alignOffsetNeeded, _size_type maxAllocatableAlignment)` furthermore
        // every Address Allocator takes `_size_type bufferSize` after the `maxAllocatableAlignment`, anything extra are the types in the `core::type_list` here
        using extra_ctor_param_types = typename AddressAlloc::extra_ctor_param_types;
        ...
        constexpr static inline bool         supportsArbitraryOrderFrees = resolve_supportsArbitraryOrderFrees<AddressAlloc>::value;
        constexpr static inline uint32_t     maxMultiOps                 = resolve_maxMultiOps<AddressAlloc>::value;
...
// TODO: the Address Allocator concept should check if the traits are instantiatiable
}
#endif