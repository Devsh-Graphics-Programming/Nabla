...
protected:
    ...
    dynamic_array_typeless_base(const allocator& _alctr, size_t _item_count)
        : alctr(_alctr), item_count(_item_count) {}
...
protected:
    inline dynamic_array(size_t _length, const allocator& _alctr = allocator())
        : base_t(_alctr, _length)
    {
        for(size_t i = 0ull; i < base_t::item_count; ++i)
            std::allocator_traits<allocator>::construct(base_t::alctr, data() + i);
    }
    inline dynamic_array(size_t _length, const T& _val, const allocator& _alctr = allocator())
        : base_t(_alctr, _length)
    {
        for(size_t i = 0ull; i < base_t::item_count; ++i)
    ...
    inline dynamic_array(const container_t& _containter, const allocator& _alctr = allocator())
        : base_t(_alctr, _containter.size())
    {
    ...
    template<typename container_t, typename iterator_t = typename container_t::iterator>
    inline dynamic_array(container_t&& _containter, const allocator& _alctr = allocator())
        : base_t(_alctr, _containter.size())
    {
        auto it = _containter.begin();
        for(size_t i = 0ull; i < base_t::item_count; ++i)
...
public:
    ...
    {
        return reinterpret_cast<T*>(static_cast<this_real_type*>(this)) + this_real_type::dummy_item_count;
    }
