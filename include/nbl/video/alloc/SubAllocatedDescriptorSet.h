...
class SubAllocatedDescriptorSet : public core::IReferenceCounted 
{
	...
	protected:
		inline ILogicalDevice* getDevice() const {return const_cast<ILogicalDevice*>(m_descriptorSet->getOriginDevice());}
	...
	public:
		// to be suballocatable
		constexpr static inline auto RequiredBindingFlags = binding_create_flags_t::ECF_UPDATE_AFTER_BIND_BIT | binding_create_flags_t::ECF_UPDATE_UNUSED_WHILE_PENDING_BIT | binding_create_flags_t::ECF_PARTIALLY_BOUND_BIT;

		// constructors
		inline SubAllocatedDescriptorSet(core::smart_refctd_ptr<video::IGPUDescriptorSet>&& descriptorSet)
		{
			assert(descriptorSet);
			auto device = const_cast<ILogicalDevice*>(descriptorSet->getOriginDevice());
			...
			for (uint32_t descriptorType = 0; descriptorType < static_cast<uint32_t>(asset::IDescriptor::E_TYPE::ET_COUNT); descriptorType++)
			{
				...
				{
					const IGPUDescriptorSetLayout::CBindingRedirect::storage_range_index_t storageIndex(i);
					const auto binding = redirect.getBinding(storageIndex);
					...
					if (flags.hasFlags(RequiredBindingFlags))
					{
						...
						auto eventHandler = std::make_unique<EventHandler>(device);
		...
		inline void multi_deallocate(uint32_t binding, size_type count, const value_type* addr, const ISemaphore::SWaitInfo& futureWait) noexcept
		{
			...
			else
			{
				...
				auto range = m_allocatableRanges.find(binding);
				getDevice()->nullifyDescriptors({nulls.data(),actualEnd});
			}
		}
