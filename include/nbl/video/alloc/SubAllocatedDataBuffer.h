...
private:
    ...
    template<class U>
    using std_get_0 = decltype(std::get<0u>(std::declval<U&>()));
    template<class, class = void>
    struct is_std_get_0_defined : std::false_type
    {
    };
    template<class U>
    struct is_std_get_0_defined<U, std::void_t<std_get_0<U> > > : std::true_type
    {
    };
...
#undef DUMMY_DEFAULT_CONSTRUCTOR
    //!
    template<typename... Args>
    SubAllocatedDataBuffer(Args&&... args)
