...
private:
    ...
    template<class U>
    using buffer_type = decltype(U::buffer);
    template<class, class = void>
    struct has_buffer_member : std::false_type
    {
    };
    template<class U>
    struct has_buffer_member<U, std::void_t<buffer_type<U>>> : std::is_same<buffer_type<U>, core::smart_refctd_ptr<IGPUBuffer>>
    {
    };
...
protected:
    ...
    public:
        ...
        inline DefaultDeferredFreeFunctor(DefaultDeferredFreeFunctor&& other)
            : sadbRef(nullptr), rangeData(nullptr), numAllocs(0u)
        {
...
public:
    ...
    //!
    template<typename... Args>
    SubAllocatedDataBuffer(ILogicalDevice* dev, Args&&... args)
        : mAllocator(std::forward<Args>(args)...), mDevice(dev)
    {
#ifdef _NBL_DEBUG
...
public:
    template<typename... Args>
    SubAllocatedDataBufferST(Args&&... args)
