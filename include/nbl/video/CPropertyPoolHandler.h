...
public:
    ...
    {
        TransferRequest()
            : download(false), pool(nullptr), indices{nullptr, nullptr}, propertyID(0xdeadbeefu)
        {
            readData = nullptr;
        }
    ...
    public:
        ...
        DeferredDescriptorSetReclaimer(DeferredDescriptorSetReclaimer&& other)
            : unusedSets(nullptr), set()
        {
...
protected:
    ...
    public:
        inline DescriptorSetCache()
            : deferredReclaims(), unusedSets(), layout(), propertyCount(0u) {}
        DescriptorSetCache(IVideoDriver* driver, uint32_t _propertyCount);
        DescriptorSetCache(const DescriptorSetCache&) = delete;
        inline DescriptorSetCache(DescriptorSetCache&& other)
            : DescriptorSetCache()
        {
    ...
    {
        inline PerPropertyCountItems()
            : descriptorSetCache(), pipeline() {}
        ...
        inline PerPropertyCountItems(PerPropertyCountItems&& other)
            : PerPropertyCountItems()
        {
            operator=(std::move(other));
        }
    ...
    {
        IndexUploadRange()
            : source{nullptr, nullptr}, destOff(0xdeadbeefu) {}
