...
#include "nbl/builtin/glsl/blit/parameters.glsl"
	...
	core::smart_refctd_ptr<IGPUSpecializedShader> createBlitSpecializedShader(
	...
	template <typename KernelX, typename KernelY, typename KernelZ>
	core::smart_refctd_ptr<IGPUComputePipeline> getBlitPipeline(
		const asset::E_FORMAT outFormat,
		const asset::IImage::E_TYPE imageType,
		const core::vectorSIMDu32& inExtent,
		const core::vectorSIMDu32& outExtent,
	...
	bool updateDescriptorSet(
		IGPUDescriptorSet* blitDS,
		IGPUDescriptorSet* kernelWeightsDS,
		core::smart_refctd_ptr<IGPUImageView> inImageView,
		core::smart_refctd_ptr<IGPUImageView> outImageView,
		core::smart_refctd_ptr<IGPUBuffer> coverageAdjustmentScratchBuffer,
		core::smart_refctd_ptr<IGPUBufferView> kernelWeightsUTB,
	...
	inline void blit(
		...
		const asset::IBlitUtilities::E_ALPHA_SEMANTIC alphaSemantic,
		IGPUDescriptorSet* alphaTestDS,
		IGPUComputePipeline* alphaTestPipeline,
		IGPUDescriptorSet* blitDS,
		IGPUDescriptorSet* blitWeightsDS,
		IGPUComputePipeline* blitPipeline,
		IGPUDescriptorSet* normalizationDS,
		IGPUComputePipeline* normalizationPipeline,
		const core::vectorSIMDu32& inImageExtent,
		const asset::IImage::E_TYPE inImageType,
		const asset::E_FORMAT inImageFormat,
		core::smart_refctd_ptr<IGPUImage> normalizationInImage,
		const KernelX& kernelX,
		const KernelY& kernelY,
		const KernelZ& kernelZ,
		const uint32_t layersToBlit = 1,
		core::smart_refctd_ptr<IGPUBuffer> coverageAdjustmentScratchBuffer = nullptr,
