...
public:
    ...
    public:
        inline DeferredCommandPoolResetter(ICommandPoolCache* _cache, const uint32_t _poolIx)
            : m_cache(_cache), m_poolIx(_poolIx)
        {
        }
        DeferredCommandPoolResetter(const DeferredCommandPoolResetter& other) = delete;
        DeferredCommandPoolResetter(DeferredCommandPoolResetter&& other)
            : m_cache(nullptr), m_poolIx(CommandPoolAllocator::invalid_address)
        {
