...
namespace nbl::video
{
	...
	public:
		...
		struct SContext {
			using AssetType = asset::IAsset;
			struct SValue
			{

				core::smart_refctd_ptr<AssetType> asset;
				core::smart_refctd_ptr<core::IReferenceCounted> gpuObj;
			};

			using hashCache_t = std::unordered_map<asset::IAsset*, size_t>;
			using cacheKey_t = std::pair<asset::IAsset*, hashCache_t*>;

			struct Hash
			{
				inline std::size_t operator()(cacheKey_t* pair) const
				{
					return pair->first->hash(pair->second);
				}
			};

			struct KeyEqual
			{
				bool operator()(cacheKey_t* lhs, cacheKey_t* rhs) const
				{
					return lhs->first->equals(rhs->first);
				}
			};

			std::list<AssetType*> conversionQueue;
			hashCache_t hashDict;
			hashCache_t hashSnapshot;
			std::unordered_map<cacheKey_t, SValue, Hash, KeyEqual> cache;
			...
			core::smart_refctd_ptr<typename video::asset_traits<asset_t>::GPUObjectType> get_cached_gpuobj(asset_t* asset) {
				return cache[std::make_pair(asset, &hashDict)].gpuObj;
			}
	...
	protected:
		...
		template<typename asset_t>
		auto call_create(asset::IAsset* ptr, SParams& params) {
			asset_t* asset = static_cast<asset_t*>(ptr);
			const asset_t** assetb = &asset;
			const  asset_t** assete = assetb + 1;
			return create(assetb, assete, params);
		}

		void create_one(core::smart_refctd_ptr<asset::IAsset> asset, SParams& params) {
			auto asset_p = asset.get();
			
			#define Case(TYPE) case TYPE::AssetType:\
				params.context.cache[std::make_pair(asset_p, &params.context.hashDict)].gpuObj = core::smart_refctd_ptr_static_cast<core::IReferenceCounted>(call_create<TYPE>(asset_p, params)[0])

			switch (asset->getAssetType())
			{
				Case(asset::ICPUBuffer);
				Case(asset::ICPUImage);
				Case(asset::ICPUSampler);
				Case(asset::ICPUPipelineLayout);
				Case(asset::ICPURenderpassIndependentPipeline);
				Case(asset::ICPUDescriptorSetLayout);
				Case(asset::ICPUComputePipeline);
				Case(asset::ICPUSpecializedShader);
				Case(asset::ICPUShader);
				Case(asset::ICPUMeshBuffer);
				Case(asset::ICPUMesh);
				Case(asset::ICPUBufferView);
				Case(asset::ICPUImageView);
				Case(asset::ICPUDescriptorSet);
				Case(asset::ICPUAnimationLibrary);
				Case(asset::ICPUAccelerationStructure);
			}
			assert(false);
		#undef Case
		}
		...
		void enqueueToConversion(core::smart_refctd_ptr<AssetType> asset, SContext<AssetType>& context) {
			...
			if (context.hashSnapshot.contains(asset.get()) {
				if (context.hashSnapshot[asset.get()] == hash &&
					context.cache[std::make_pair(asset.get(), &(context.hashDict))].gpuObj != nullptr) {
					return;
				}
			}
			if (context.cache.contains(std::make_pair(asset.get(), &(context.hashDict)))) {
				return;
			}
			else {
				context.cache.insert(std::make_pair(asset.get(), &(context.hashDict)), { asset, nullptr })
			}
		...
		{
			{
				...
				{
					auto asset_p = (*iter).second.asset;
					auto recalculatedHash = asset_p->hash(_context.hashDict);
					bool evict = _context.hashSnapshot.contains(asset_p.get()) && _context.hashSnapshot[asset_p.get()] == recalculatedHash;
