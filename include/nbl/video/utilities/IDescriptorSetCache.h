...
public:
    ...
    public:
        inline DeferredDescriptorSetReclaimer(IDescriptorSetCache* _cache, const uint32_t _setIx)
            : m_cache(_cache), m_setIx(_setIx)
        {
        }
        ...
        DeferredDescriptorSetReclaimer(DeferredDescriptorSetReclaimer&& other)
            : m_cache(nullptr), m_setIx(DescSetAllocator::invalid_address)
        {
