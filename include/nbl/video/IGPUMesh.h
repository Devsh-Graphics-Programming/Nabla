...
public:
    IGPUMesh(uint32_t meshBufferCount)
        : m_meshBuffers(core::make_refctd_dynamic_array<MeshBufferRefContainer>(meshBufferCount))
    {
    }

    template<typename MeshBufferIterator>
    IGPUMesh(MeshBufferIterator begin, MeshBufferIterator end)
        : IGPUMesh(std::distance(begin, end))
    {
