...
public:
    ...
    {
        Keyframe()
            : scale(0ull)  // TODO: initialize scale to 1.f
        {
...
protected:
    ...
    inline void addAnimationNames(NameIterator nameBegin, NameIterator nameEnd, OffsetIterator offsetBegin)
    {
        ...
        for(auto it = nameBegin; it != nameEnd; it++, offsetIt++)
        {
            ...
            while(*inName)
            {
                m_stringPool[stringPoolEnd++] = *(inName++);
            }
    ...
    {
        StringComparator(const core::vector<char>* const _stringPool, const char* const* _temporaryString)
            : stringPool(_stringPool), temporaryString(_temporaryString) {}
