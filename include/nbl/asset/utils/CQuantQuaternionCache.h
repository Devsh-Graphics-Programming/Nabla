...
{ 
	public:
		...
		struct alignas(uint64_t) Vector16u
		{
			uint16_t x;
			uint16_t y;
			uint16_t z;
		...
		struct alignas(uint32_t) Vector8u
		{
			uint8_t x;
			uint8_t y;
			uint8_t z;
	...
	private:
		struct QuantQuaternionHash
		{
			inline size_t operator()(const Projection& vec) const noexcept
			{
				...
				static constexpr size_t primeNumber3 = 69ull; // until @Przemog suggests something nice
	...
	public:
		template<E_CACHE_TYPE CacheType>
		vector_for_cache_t<CacheType> quantizeQuaternion(const core::quaternion& quat)
		{
			uint32_t quantizationBits = 0;
			if constexpr(CacheType == E_CACHE_TYPE::ECT_8_8_8_8)
			{
				quantizationBits = 8u;
			}
			else (CacheType == E_CACHE_TYPE::ECT_16_16_16_16)
			{
			...
			const auto negativeMask = quat < core::vectorSIMDf(0.0f);

			core::vectorSIMDf absQuat = abs(quat);
...
template<> 
struct CQuantNormalCache::vector_for_cache<CQuantNormalCache::E_CACHE_TYPE::ECT_8_8_8_8>
{
...
template<> 
struct CQuantNormalCache::vector_for_cache<CQuantNormalCache::E_CACHE_TYPE::ECT_16_16_16_16>
{
