...
{    
    public:
        ...
        class DefaultWeldPredicate final : public WeldPredicate
        {
            ...
            public:
                ...
                inline bool init(const ICPUPolygonGeometry* polygon) override
                {
                    ...
                    if (IGeometryBase::getMatchingAABBFormat(positionView.composed.format) != positionView.composed.rangeFormat)
                        return false;
                    ...
                    if (normalView && !isViewFormatValid(normalView))
                        return false;
                    ...
                    {
                        if (jointWeightView.indices && !isViewFormatValid(jointWeightView.indices))
                            return false;
                        if (jointWeightView.weights && !isViewFormatValid(jointWeightView.weights))
                            return false;
                    ...
                    {
                        if (auxAttributeView && !isViewFormatValid(auxAttributeView))
                            return false;
                ...
                inline bool operator()(const ICPUPolygonGeometry* polygon, uint32_t index1, uint32_t index2) const override
                {
                    ...
                    for (uint64_t joint_i = 0; joint_i < polygon->getJointWeightViews().size(); joint_i++)
                    {
                        ...
                        if (!isAttributeValEqual(jointWeightView.indices, m_jointViewContexts[joint_i].indices, index1, index2, m_epsilon))
                            return false;
                        if (!isAttributeValEqual(jointWeightView.weights, m_jointViewContexts[joint_i].weights, index1, index2, m_epsilon))
                            return false;
                    }

                    const auto& auxAttrViews = polygon->getAuxAttributeViews();
                    for (uint64_t aux_i = 0; aux_i < auxAttrViews.size(); aux_i++)
                    if (!isAttributeValEqual(auxAttrViews[aux_i], m_auxAttributeViewContexts[aux_i], index1, index2, m_epsilon))
                        return false;
        ...
        static inline core::smart_refctd_ptr<ICPUPolygonGeometry> weldVertices(const ICPUPolygonGeometry* polygon, const AccelStructureT& as, WeldPredicate& shouldWeldFn, const bool recomputeHash=true)
        {
              auto outPolygon = core::move_and_static_cast<ICPUPolygonGeometry>(polygon->clone(0u));

              if (!shouldWeldFn.init(polygon))
                  return nullptr;
