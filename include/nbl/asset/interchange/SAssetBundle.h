...
public:
    ...
    SAssetBundle(const size_t assetCount = 0ull)
        : m_metadata(nullptr), m_contents(core::make_refctd_dynamic_array<contents_container_t>(assetCount)), m_cacheKey("")
    {
    }
    SAssetBundle(core::smart_refctd_ptr<IAssetMetadata>&& _metadata, contents_container_t&& _contents)
        : m_metadata(std::move(_metadata)), m_contents(std::move(_contents)), m_cacheKey{}
    {
