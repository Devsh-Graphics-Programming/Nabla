	...
	protected:
		...
		class CFileMemoryResource final : public core::refctd_memory_resource
		{
			public:
				...
				{
					assert(m_file);
					[[maybe_unused]] auto* const basePtr = reinterpret_cast<const uint8_t*>(m_file->getMappedPointer());
		...
		{
			if ([[maybe_unused]] auto* const basePtr=reinterpret_cast<const uint8_t*>(file->getMappedPointer()); basePtr)
			{
