...
class IBuffer : public core::IBuffer, public IDescriptor
{
	public:
		...
		enum E_USAGE_FLAGS : uint32_t
		{
			...
			EUF_ANY = EUF_TRANSFER_SRC_BIT
					| EUF_TRANSFER_DST_BIT
					| EUF_UNIFORM_TEXEL_BUFFER_BIT
					| EUF_STORAGE_TEXEL_BUFFER_BIT
					| EUF_UNIFORM_BUFFER_BIT
					| EUF_STORAGE_BUFFER_BIT
					| EUF_INDEX_BUFFER_BIT
					| EUF_VERTEX_BUFFER_BIT
					| EUF_INDIRECT_BUFFER_BIT
					| EUF_SHADER_DEVICE_ADDRESS_BIT
					| EUF_CONDITIONAL_RENDERING_BIT_EXT
					| EUF_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT
					| EUF_ACCELERATION_STRUCTURE_STORAGE_BIT
					| EUF_SHADER_BINDING_TABLE_BIT
        };

		static constexpr uint32_t USAGE_COUNT = std::popcount(static_cast<uint32_t>(EUF_ANY));
