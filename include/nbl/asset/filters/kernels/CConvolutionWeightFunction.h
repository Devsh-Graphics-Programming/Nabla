...
struct convolution_weight_function_helper
{
	...
	inline static float operator_impl(const CConvolutionWeightFunction<Weight1DFunctionA, Weight1DFunctionB>& _this, const float x, const uint32_t channel, const uint32_t sampleCount)
	{
		if constexpr (std::is_same_v<Weight1DFunctionB, SDiracFunction>)
		{
			return _this.m_funcA.operator() < derivative > (x, channel);
		}
		else if (std::is_same_v<Weight1DFunctionA, SDiracFunction>)
		{
		...
		else
		{
			...
			if (minIntegrationLimit == maxIntegrationLimit)
				return _this.m_funcA.operator()<derivative>(x, channel) * Weight1DFunctionB::k_energy[channel];
			...
			for (uint32_t i = 0u; i < sampleCount; ++i)
			{
				...
				result += _this.m_funcA.operator()<deriv_A>(x - t, channel) * _this.m_funcB.operator()<deriv_B>(t * _this.m_ratio, channel) * dt;
			}
...
template <>
struct convolution_weight_function_helper<SBoxFunction, SBoxFunction>
{
	template <int32_t derivative>
	static float operator_impl(const CConvolutionWeightFunction<SBoxFunction, SBoxFunction>& _this, const float x, const uint32_t channel, const uint32_t sampleCount);
};

template <>
struct convolution_weight_function_helper<SGaussianFunction, SGaussianFunction>
{
	template <int32_t derivative>
	static float operator_impl(const CConvolutionWeightFunction<SGaussianFunction, SGaussianFunction>& _this, const float x, const uint32_t channel, const uint32_t sampleCount);
};

// TODO: Specialization: convolution_weight_function_helper<Triangle,Triangle> = this is tricky but feasible

template <>
struct convolution_weight_function_helper<SKaiserFunction, SKaiserFunction>
{
	template <int32_t derivative>
	static float operator_impl(const CConvolutionWeightFunction<SKaiserFunction, SKaiserFunction>& _this, const float x, const uint32_t channel, const uint32_t sampleCount);
};
...
public:
	...
	{
		return impl::convolution_weight_function_helper<derivative>::operator_impl(*this, x, channel, sampleCount);
	}
