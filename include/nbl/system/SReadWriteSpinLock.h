...
public:
    ...
    rw_lock_guard_base(rw_lock_guard_base&& rhs)
        : rw_lock_guard_base()
    {
...
protected:
    rw_lock_guard_base(SReadWriteSpinLock& lk)
        : m_lock(&lk) {}
...
{
public:
    read_lock_guard(SReadWriteSpinLock& lk, std::adopt_lock_t)
        : impl::rw_lock_guard_base(lk) {}
    explicit read_lock_guard(SReadWriteSpinLock& lk)
        : read_lock_guard(lk, std::adopt_lock_t())
    {
...
{
public:
    write_lock_guard(SReadWriteSpinLock& lk, std::adopt_lock_t)
        : impl::rw_lock_guard_base(lk) {}
    explicit write_lock_guard(SReadWriteSpinLock& lk)
        : write_lock_guard(lk, std::adopt_lock_t())
    {
...
inline read_lock_guard<LoadOrder, ReadModWriteOrder>::read_lock_guard(write_lock_guard<LoadOrder, ReadModWriteOrder>&& wl)
    : impl::rw_lock_guard_base(std::move(wl))
{
    m_lock->m_lock.fetch_sub(impl::SReadWriteSpinLockBase::LockWriteVal - 1u, ReadModWriteOrder);
}

template<std::memory_order LoadOrder, std::memory_order ReadModWriteOrder>
inline write_lock_guard<LoadOrder, ReadModWriteOrder>::write_lock_guard(read_lock_guard<LoadOrder, ReadModWriteOrder>&& rl)
    : impl::rw_lock_guard_base(std::move(rl))
{
