...
public:
    ILogger(core::bitflag<E_LOG_LEVEL> logLevelMask)
...
protected:
    ...
    virtual std::string constructLogString(const std::string_view& fmtString, E_LOG_LEVEL logLevel, va_list l)
    {
        ...
        while(time_since_epoch.count() / 100000 == 0)
            time_since_epoch *= 10;
...
{
public:
    logger_opt_ptr(ILogger* const _logger)
        : logger(_logger) {}
...
{
public:
    logger_opt_smart_ptr(core::smart_refctd_ptr<ILogger>&& _logger)
        : logger(std::move(_logger)) {}
    logger_opt_smart_ptr(std::nullptr_t t)
        : logger(nullptr) {}
