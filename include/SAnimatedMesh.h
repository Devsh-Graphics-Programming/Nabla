...
namespace scene
{
	...
	class SCPUAnimatedMesh : public ICPUAnimatedMesh
	{
		//! constructor
		SCPUAnimatedMesh(scene::ICPUMesh* mesh=0, scene::E_ANIMATED_MESH_TYPE type=scene::EAMT_UNKNOWN) : FramesPerSecond(25.f), Type(type)
		{
			#ifdef _DEBUG
			setDebugName("SCPUAnimatedMesh");
			#endif
			addMesh(mesh);
		}

		//! destructor
		virtual ~SCPUAnimatedMesh()
		{
			// drop meshes
	...
	{
		//! constructor
		SAnimatedMesh(scene::IGPUMesh* mesh=0, scene::E_ANIMATED_MESH_TYPE type=scene::EAMT_UNKNOWN) : IAnimatedMesh(), FramesPerSecond(25.f), Type(type)
		{
			#ifdef _DEBUG
