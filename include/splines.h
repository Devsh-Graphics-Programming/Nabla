...
{
public:
    CLinearSpline(vectorSIMDf* controlPoints, const size_t& count, const bool loop = false)
        : ISpline(loop)
    {
    ...
    CLinearSpline(vectorSIMDf* controlPoints, float* customDistances, const size_t& count, const bool loop = false)
        : ISpline(loop)
    {
...
{
public:
    CQuadraticSpline(vectorSIMDf* controlPoints, const size_t& count, const bool loop = false, float tightness = 1.1107f)
        : ISpline(loop)
    {
        assert(count > 1ull);
...
{
public:
    CQuadraticBSpline(vectorSIMDf* controlPoints, const size_t& count, const bool loop = false)
        : CQuadraticSpline(loop)
    {
        //assert(count<0x80000000u && count);
