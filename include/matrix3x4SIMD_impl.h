...
inline matrix3x4SIMD& matrix3x4SIMD::setScale(const core::vectorSIMDf& _scale)
{
	...
	rowAlias0 = (scaleAlias & vectorSIMDf(BUILD_MASKF(1, 0, 0, 0))) | (rowAlias0 & vectorSIMDf(mask0001));
	rowAlias1 = (scaleAlias & vectorSIMDf(BUILD_MASKF(0, 1, 0, 0))) | (rowAlias1 & vectorSIMDf(mask0001));
	rowAlias2 = (scaleAlias & vectorSIMDf(BUILD_MASKF(0, 0, 1, 0))) | (rowAlias2 & vectorSIMDf(mask0001));
...
inline matrix3x4SIMD& matrix3x4SIMD::setRotation(const core::quaternion& _quat)
{
	...
	rows[0] = ((quat.yyyy() * ((quat.yxwx() & mask1110) * vectorSIMDf(2.f))) + (quat.zzzz() * (quat.zwxx() & mask1110) * vectorSIMDf(2.f, -2.f, 2.f, 0.f))) | (reinterpret_cast<const vectorSIMDu32&>(rows[0]) & vectorSIMDf(mask0001));
	rows[0].x = 1.f - rows[0].x;

	rows[1] = ((quat.zzzz() * ((quat.wzyx() & mask1110) * vectorSIMDf(2.f))) + (quat.xxxx() * (quat.yxwx() & mask1110) * vectorSIMDf(2.f, 2.f, -2.f, 0.f))) | (reinterpret_cast<const vectorSIMDu32&>(rows[1]) & vectorSIMDf(mask0001));
	rows[1].y = 1.f - rows[1].y;

	rows[2] = ((quat.xxxx() * ((quat.zwxx() & mask1110) * vectorSIMDf(2.f))) + (quat.yyyy() * (quat.wzyx() & mask1110) * vectorSIMDf(-2.f, 2.f, 2.f, 0.f))) | (reinterpret_cast<const vectorSIMDu32&>(rows[2]) & vectorSIMDf(mask0001));
