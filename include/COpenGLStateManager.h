...
public:
    COpenGLStateDiff()
        : hintsToSet(0), glProvokingVertex_val(GL_INVALID_ENUM), glEnableCount(0), glDisableCount(0), glDisableiCount(0), glEnableiCount(0),
    ...
    public:
        IndexedObjectToBind()
            : index(0xdeadbeefu), obj(0)
        {
        }
        IndexedObjectToBind(const uint32_t ix, const GLuint object)
            : index(ix), obj(object)
        {
        }
    ...
    {
    public:
        RangedBufferBindingDiff()
            : RangedBufferBinding(), index(0xdeadbeefu), bindPoint(GL_INVALID_ENUM)
        {
        }
    ...
    public:
        TextureToBind()
            : IndexedObjectToBind(), target(GL_INVALID_ENUM)
        {
        }
        TextureToBind(const uint32_t ix, const GLuint object, const GLenum tgt)
            : IndexedObjectToBind(ix, object), target(tgt)
        {
        }
