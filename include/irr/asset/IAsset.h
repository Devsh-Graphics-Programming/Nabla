...
namespace irr {
	namespace asset
	{
		...
		public:
			...
			template<class container_t = core::vector<core::smart_refctd_ptr<IAsset>>>
			SAssetBundle(container_t&& _contents) : m_contents(new contents_container_t(_contents.size(), nullptr), core::dont_grab)
			{
				std::move(_contents.begin(), _contents.end(), m_contents->begin());
				assert(allSameTypeAndNotNull());
			}
