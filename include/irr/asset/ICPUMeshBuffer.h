...
public:
    ...
    size_t calcVertexSize() const
    {
        ...
        for (size_t i = 0; i < scene::EVAI_COUNT; ++i)
            if (meshLayout->getMappedBuffer((scene::E_VERTEX_ATTRIBUTE_ID)i))
                size += scene::vertexAttrSize[meshLayout->getAttribType((scene::E_VERTEX_ATTRIBUTE_ID)i)][meshLayout->getAttribComponentCount((scene::E_VERTEX_ATTRIBUTE_ID)i)];
        return size;
    }
    ...
    */
    virtual uint8_t* getAttribPointer(const scene::E_VERTEX_ATTRIBUTE_ID& attrId) const
    {
        ...
        scene::E_COMPONENT_TYPE attrType = meshLayout->getAttribType(attrId);
        scene::E_COMPONENTS_PER_ATTRIBUTE components = meshLayout->getAttribComponentCount(attrId);
    ...
    static bool getAttribute(core::vectorSIMDf& output, const void* src, scene::E_COMPONENT_TYPE type, scene::E_COMPONENTS_PER_ATTRIBUTE cpa)
    {
        if (!src)
            return false;
        ...
        alignas(std::alignment_of<core::vectorSIMD_32<uint32_t> >::value) const uint32_t outMasksBits[scene::ECPA_COUNT][4] = {
        ...
        alignas(std::alignment_of<core::vectorSIMD_32<uint32_t> >::value) const uint32_t outMasksBitsInv[scene::ECPA_COUNT][4] = {
            { 0,0,0,0 },
    ...
    */
    ...
    static bool getAttribute(uint32_t* output, const void* src, scene::E_COMPONENT_TYPE type, scene::E_COMPONENTS_PER_ATTRIBUTE cpa)
    {
        if (!src)
            return false;
        const uint8_t* vxPtr = (const uint8_t*)(src);
    ...
    */
    ...
    static bool setAttribute(core::vectorSIMDf input, void* dst, scene::E_COMPONENT_TYPE type, scene::E_COMPONENTS_PER_ATTRIBUTE cpa)
    {
        if (!dst)
            return false;
    ...
    */
    ...
    static bool setAttribute(const uint32_t* _input, void* dst, scene::E_COMPONENT_TYPE type, scene::E_COMPONENTS_PER_ATTRIBUTE cpa)
    {
        if (!dst)
            return false;
        uint8_t* vxPtr = (uint8_t*)dst;
