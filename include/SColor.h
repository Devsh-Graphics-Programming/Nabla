...
public:
    ...
    SColorf(float r_in, float g_in, float b_in, float a_in = 1.0f)
        : vectorSIMDf(r_in, g_in, b_in, a_in) {}

    SColorf(const vectorSIMDf& fromVec4)
        : vectorSIMDf(fromVec4) {}
    ...
    inline void set(float aa, float rr, float gg, float bb)
    {
        a = aa;
        r = rr;
        g = gg;
