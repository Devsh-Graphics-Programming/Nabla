...
**/
	...
	*/
	inline E_COMPONENT_TYPE getCorrespondingNotNormalizedType(E_COMPONENT_TYPE _type)
	{
		switch (_type)
		{
		case ECT_NORMALIZED_INT_2_10_10_10_REV:
...
*/
	...
	public:
		...
		*/
		...
		static bool getAttribute(core::vectorSIMDf& output, const void* src, E_COMPONENT_TYPE type, E_COMPONENTS_PER_ATTRIBUTE cpa, size_t ix, size_t stride, const void* end)
		{
			if (!src)
				return false;
		...
		*/
        virtual bool getAttribute(core::vectorSIMDf& output, const E_VERTEX_ATTRIBUTE_ID& attrId, size_t ix) const
        {
			...
			if (!mappedAttrBuf)
				return false;
			return getAttribute(output, (void*)getAttribPointer(attrId), meshLayout->getAttribType(attrId), meshLayout->getAttribComponentCount(attrId), ix, meshLayout->getMappedBufferStride(attrId), (const void*)(((const uint8_t*)(mappedAttrBuf->getPointer()))+mappedAttrBuf->getSize()));
        }

		static bool getAttribute(uint32_t* output, const void* src, E_COMPONENT_TYPE type, E_COMPONENTS_PER_ATTRIBUTE cpa, size_t ix, size_t stride, const void* end)
		{
			if (!src)
				return false;
			const uint8_t* vxPtr = (const uint8_t*)(src);
		...
		*/
		virtual bool getAttribute(uint32_t* output, const E_VERTEX_ATTRIBUTE_ID& attrId, size_t ix) const
		{
			...
			if (!mappedAttrBuf)
				return false;
			return getAttribute(output, getAttribPointer(attrId), meshLayout->getAttribType(attrId), meshLayout->getAttribComponentCount(attrId), ix, meshLayout->getMappedBufferStride(attrId), (const void*)(((const uint8_t*)(mappedAttrBuf->getPointer())) + mappedAttrBuf->getSize()));
		}

		static bool setAttribute(core::vectorSIMDf input, void* dst, E_COMPONENT_TYPE type, E_COMPONENTS_PER_ATTRIBUTE cpa, size_t ix, size_t stride, const void* end)
		{
			if (!dst)
				return false;
		...
		*/
        virtual bool setAttribute(core::vectorSIMDf input, const E_VERTEX_ATTRIBUTE_ID& attrId, size_t ix) const
        {
			...
			if (!mappedBuffer)
				return false;
			return setAttribute(input, getAttribPointer(attrId), meshLayout->getAttribType(attrId), meshLayout->getAttribComponentCount(attrId), ix, meshLayout->getMappedBufferStride(attrId), ((uint8_t*)(mappedBuffer->getPointer()))+mappedBuffer->getSize());
        }

		static bool setAttribute(const uint32_t* _input, void* dst, E_COMPONENT_TYPE type, E_COMPONENTS_PER_ATTRIBUTE cpa, size_t ix, size_t stride, const void* end)
		{
			if (!dst)
				return false;
			uint8_t* vxPtr = (uint8_t*)dst;
        ...
        virtual bool setAttribute(const uint32_t* _input, const E_VERTEX_ATTRIBUTE_ID& attrId, size_t ix) const
        {
			...
			if (!mappedBuffer)
				return false;
			return setAttribute(_input, getAttribPointer(attrId), meshLayout->getAttribType(attrId), meshLayout->getAttribComponentCount(attrId), ix, meshLayout->getMappedBufferStride(attrId), ((const uint8_t*)(mappedBuffer->getPointer()))+mappedBuffer->getSize());
        }
