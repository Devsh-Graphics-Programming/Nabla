		...
		*/
        virtual bool getAttribute(core::vectorSIMDf& output, const E_VERTEX_ATTRIBUTE_ID& attrId, size_t ix) const
        {
            ...
            switch (attrType)
            {
                ...
                case ECT_HALF_FLOAT:
					for (size_t j = 0; j < components; ++j)
						output.pointer[j] = Float16Compressor::decompress(((uint16_t*)vxPtr)[j]);
					return true;
					break;
                ...
                case ECT_UNSIGNED_INT_10F_11F_11F_REV:
					{
						...
						for (size_t j = 0; j < 3; ++j)
						{
							if (j < 2)
							{
								output.pointer[j] = unpack11bitFloat(fp);
								fp >>= 11;
							}
							else
								output.pointer[j] = unpack10bitFloat(fp);
						}
					}
                    break;
		...
		virtual bool getAttribute(uint32_t* output, const E_VERTEX_ATTRIBUTE_ID& attrId, size_t ix) const
		{
			...
			case ECT_UNSIGNED_INT_10F_11F_11F_REV:
			{
				...
				for (size_t j = 0; j < 3; ++j)
				{
					if (j < 2)
					{
						output[j] = unpack11bitFloat(fp);
						fp >>= 11;
					}
					else
						output[j] = unpack10bitFloat(fp);
				}
			}
			break;
			case ECT_INT_2_10_10_10_REV:
		...
		*/
        virtual bool setAttribute(core::vectorSIMDf input, const E_VERTEX_ATTRIBUTE_ID& attrId, size_t ix) const
        {
            ...
            switch (attrType)
            {
                ...
                case ECT_HALF_FLOAT:
					for (size_t j = 0; j < components; ++j)
						((uint16_t*)vxPtr)[j] = Float16Compressor::compress(input.pointer[j]); // store on 2 bytes, padded to 4 bytes
                    return true;
                    break;
                case ECT_DOUBLE_IN_FLOAT_OUT:
                case ECT_DOUBLE_IN_DOUBLE_OUT:
                ...
                case ECT_UNSIGNED_INT_10F_11F_11F_REV:
					if (components != ECPA_THREE)
						return false;
					((uint32_t*)vxPtr)[0] =
						(to11bitFloat(input.pointer[0]) << 0)  |
						(to11bitFloat(input.pointer[1]) << 11) |
						(to10bitFloat(input.pointer[2]) << 22);
                    return true;
                    break;
				case ECT_NORMALIZED_INT_2_10_10_10_REV:
				case ECT_NORMALIZED_UNSIGNED_INT_2_10_10_10_REV:
		...
		private:
			/** Floating point number in unsigned 11-bit floating point format shall be on eleven youngest bits */
			float unpack11bitFloat(uint32_t _fp) const
			{
			...
			uint32_t to11bitFloat(float _f32) const
			{
			...
			/** Floating point number in unsigned 10-bit floating point format shall be on ten youngest bits */
			float unpack10bitFloat(uint32_t _fp) const
			{
			...
			uint32_t to10bitFloat(float _f32) const
			{
				const uint32_t& f32 = *((uint32_t*)&_f32);
