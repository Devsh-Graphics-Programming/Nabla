...
**/
	...
	*/
	...
	/** @param type Type of an attribute.
	@returns true if given attribute type is "weak ineteger" or false otherwise.
	*/
...
*/
	...
	public:
		...
		{
			ICPUMeshBuffer* output = new ICPUMeshBuffer(*this);
		...
		*/
		...
		protected:
			void copyVertexDataFromThis(ICPUMeshBuffer* _dst, bool _destIsShallowCopyOfThis) const
			{
				if (!this->getMeshDataAndFormat())
					return;

				core::ICPUBuffer* idxBuffer = NULL;
				if (this->getIndices())
				{
					idxBuffer = new core::ICPUBuffer(this->getMeshDataAndFormat()->getIndexBuffer()->getSize());
					memcpy(idxBuffer->getPointer(), this->getIndices(), idxBuffer->getSize());
				}

				ICPUMeshDataFormatDesc* desc = new ICPUMeshDataFormatDesc();
				const IMeshDataFormatDesc<core::ICPUBuffer>* oldDesc = this->getMeshDataAndFormat();

				std::map<const core::ICPUBuffer*, E_VERTEX_ATTRIBUTE_ID> buffers;
				std::set<core::ICPUBuffer*> newBuffers;
				for (size_t i = 0; i < EVAI_COUNT; ++i)
				{
					...
					std::map<const core::ICPUBuffer*, E_VERTEX_ATTRIBUTE_ID>::iterator itr = buffers.find(buf);
					if (itr == buffers.end())
					{
						buffers[buf] = (E_VERTEX_ATTRIBUTE_ID)i;
						newBuf = new core::ICPUBuffer(buf->getSize());
						memcpy(newBuf->getPointer(), buf->getPointer(), newBuf->getSize());
					}
					else
					{
						newBuf = const_cast<core::ICPUBuffer*>(desc->getMappedBuffer(itr->second));
					}

					desc->mapVertexAttrBuffer(newBuf, (E_VERTEX_ATTRIBUTE_ID)i,
					...
					newBuffers.insert(newBuf);
				}
				if (idxBuffer)
				{
					desc->mapIndexBuffer(idxBuffer);
					idxBuffer->drop();
				}
				for (std::set<core::ICPUBuffer*>::iterator it = newBuffers.begin(); it != newBuffers.end(); ++it)
					(*it)->drop();

				if (_destIsShallowCopyOfThis)
					oldDesc->grab();
				_dst->setMeshDataAndFormat(desc);
			}
	};
