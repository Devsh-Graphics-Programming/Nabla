	...
	class IGPUMeshDataFormatDesc : public IMeshDataFormatDesc<video::IGPUBuffer>
	{
	    public:
            struct HashAttribs
            {
                ...
                inline void setAttrFmtAndCompCnt(const scene::E_VERTEX_ATTRIBUTE_ID& attrId, const E_COMPONENTS_PER_ATTRIBUTE& components, const E_COMPONENT_TYPE& type)
                {
                ...
                union
                {
                    ...
                    struct
                    {
                        ...
                        uint32_t elementArrayBinding;
                        uint16_t enabledAttribs;
                        uint16_t attributeDivisors[_IRR_VAO_MAX_ATTRIB_DIVISOR_BITS];
                    } PACK_STRUCT;
                    #include "irrunpack.h"

                    uint64_t hashVal[(scene::EVAI_COUNT+4+2+2*_IRR_VAO_MAX_ATTRIB_DIVISOR_BITS+sizeof(uint64_t)-1)/sizeof(uint64_t)];
                };
            };
            ...
            **/
            virtual const HashAttribs* getHash() = 0;


        protected:
            HashAttribs individualHashFields;

            virtual ~IGPUMeshDataFormatDesc()
            {
	        ...
	        IGPUMeshDataFormatDesc()
	        {
                ...
                static_assert(sizeof(HashAttribs)/sizeof(uint64_t)==(22+_IRR_VAO_MAX_ATTRIB_DIVISOR_BITS*2+sizeof(uint64_t)-1)/sizeof(uint64_t)); //otherwise our hashing system falls apart
                for (size_t i=0; i<scene::EVAI_COUNT; i++)
                    individualHashFields.setAttrFmtAndCompCnt(static_cast<E_VERTEX_ATTRIBUTE_ID>(i),compntsPerAttr[i],attrType[i]);
	        }
	};
