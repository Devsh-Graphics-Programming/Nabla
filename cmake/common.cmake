...
function(nbl_project_handle_json_config)
	...
	function(NBL_JSON_UPDATE_RELATIVE_REFERENCE_FILEPATH)
		message(FATAL_ERROR "TODO: GET NAME OF OUTPUT STREAM VARIABLE FROM ARG OF THE UTILITY")
		...
		macro(NBL_IMPL_JSON_UPDATE_RELATIVE_REFERENCE_FILEPATH _NBL_IMPL_JSON_READ_REFERENCE_FILEPATH_CONTENT_)
			...
			if(EXISTS "${_NBL_JSON_REFERENCE_FILEPATH_ABS_}")
				...
				string(JSON _NBL_GEN_JSON_TOP_CONFIG_CONTENT_ ERROR_VARIABLE _NBL_JSON_ERROR_ SET "${_NBL_GEN_JSON_TOP_CONFIG_CONTENT_}" ${_NBL_MEMBER_INDEX_ARGV_} "\"${_NBL_JSON_GEN_REFERENCE_FILEPATH_}\"") # TODO: GET NAME OF OUTPUT STREAM VARIABLE FROM ARG OF THE UTILITY
				set(_NBL_GEN_JSON_TOP_CONFIG_CONTENT_ "${_NBL_GEN_JSON_TOP_CONFIG_CONTENT_}" PARENT_SCOPE) # TODO: GET NAME OF OUTPUT STREAM VARIABLE FROM ARG OF THE UTILITY
			endif()
		endmacro()
	...
	if(EXISTS "${_NBL_JSON_CONFIG_FILEPATH_}")		
		...
		NBL_JSON_UPDATE_RELATIVE_REFERENCE_FILEPATH(_NBL_GEN_JSON_TOP_CONFIG_CONTENT_ "${_NBL_JSON_TOP_CONFIG_CONTENT_}" scriptPath)
		...
		macro(NBL_JSON_HANDLE_INPUT_CONFIG_PART _NBL_JSON_GEN_CONTENT_VARIABLE_NAME_ _NBL_INPUT_JSON_CONFIG_FILEPATH_ _NBL_INPUT_JSON_CONFIG_CONTENT_)
			...
			# TODO: STREAM UPDATE TO GIVEN VARIABLE GIVEN BY ARY OF THE MACRO
			NBL_JSON_UPDATE_RELATIVE_REFERENCE_FILEPATH("${_NBL_JSON_GEN_CONTENT_VARIABLE_NAME_}" "${_NBL_JSON_TOP_CONFIG_CONTENT_}" dependencies)
			...
			if(_NBL_JSON_DATA_LEN_ GREATER_EQUAL 1)
				...
				foreach(_NBL_IDX_ RANGE ${_NBL_STOP_})
					...
					if(_NBL_JSON_COMMAND_LEN_ GREATER_EQUAL 1)
						...
						# TODO: STREAM UPDATE TO GIVEN VARIABLE GIVEN BY ARY OF THE MACRO
						NBL_JSON_UPDATE_RELATIVE_REFERENCE_FILEPATH("${_NBL_JSON_GEN_CONTENT_VARIABLE_NAME_}" "${_NBL_JSON_TOP_CONFIG_CONTENT_}" data ${_NBL_IDX_} command)
					...
					# TODO: STREAM UPDATE TO GIVEN VARIABLE GIVEN BY ARY OF THE MACRO
					NBL_JSON_UPDATE_RELATIVE_REFERENCE_FILEPATH("${_NBL_JSON_GEN_CONTENT_VARIABLE_NAME_}" "${_NBL_JSON_TOP_CONFIG_CONTENT_}" data ${_NBL_IDX_} dependencies)
				endforeach()
			endif()
		endmacro()
		...
		if(_NBL_JSON_INPUTS_LEN_ GREATER_EQUAL 1)
			...
			foreach(_NBL_IDX_ RANGE ${_NBL_STOP_})
				...
				if(EXISTS "${_NBL_EXT_INPUT_JSON_CONFIG_FILEPATH_}")		
					...
					NBL_JSON_HANDLE_INPUT_CONFIG_PART(_NBL_GEN_EXT_INPUT_JSON_CONFIG_CONTENT_ "${_NBL_EXT_INPUT_JSON_CONFIG_FILEPATH_}" "${_NBL_EXT_INPUT_JSON_CONFIG_CONTENT_}") # TODO: PERFORM TESTS
				
					# TODO: GIVEN _NBL_GEN_EXT_INPUT_JSON_CONFIG_CONTENT_ UPDATE MAIN GEN JSON CONFIG, MERGE CONTENT AND RESOLVE PATHS 
				else()
