...
function(nbl_install_example_runtime_component)
	set(REQUIRED_SINGLE_ARGS COMPONENT)
	set(OPTIONAL_SINGLE_ARGS TARGET DISPLAY_NAME DESCRIPTION)
	cmake_parse_arguments(IMPL
		"NO_TARGET"
		"${REQUIRED_SINGLE_ARGS};${OPTIONAL_SINGLE_ARGS}"
		"DIRECTORIES;FILES;GENERATED_RUNTIME_FILES"
		${ARGV}
	)
	NBL_PARSE_REQUIRED(IMPL ${REQUIRED_SINGLE_ARGS})

	if(IMPL_UNPARSED_ARGUMENTS)
		message(FATAL_ERROR "nbl_install_example_runtime_component got unexpected arguments: ${IMPL_UNPARSED_ARGUMENTS}")
	endif()

	if(IMPL_KEYWORDS_MISSING_VALUES)
		message(FATAL_ERROR "nbl_install_example_runtime_component got keywords with missing values: ${IMPL_KEYWORDS_MISSING_VALUES}")
	endif()

	if(NOT IMPL_TARGET)
		if(DEFINED EXECUTABLE_NAME)
			set(IMPL_TARGET "${EXECUTABLE_NAME}")
		else()
			message(FATAL_ERROR "nbl_install_example_runtime_component requires TARGET when EXECUTABLE_NAME is not defined.")
		endif()
	endif()

	if(NOT TARGET ${IMPL_TARGET})
		message(FATAL_ERROR "nbl_install_example_runtime_component target does not exist: ${IMPL_TARGET}")
	endif()

	get_target_property(_NBL_EXAMPLE_RUNTIME_SOURCE_DIR "${IMPL_TARGET}" SOURCE_DIR)
	file(RELATIVE_PATH _NBL_EXAMPLE_RUNTIME_RELATIVE_DIR "${NBL_ROOT_PATH}" "${_NBL_EXAMPLE_RUNTIME_SOURCE_DIR}")
	set(_NBL_EXAMPLE_RUNTIME_BIN_DESTINATION "$<$<NOT:$<STREQUAL:$<CONFIG>,Release>>:$<LOWER_CASE:$<CONFIG>>/>exe/${_NBL_EXAMPLE_RUNTIME_RELATIVE_DIR}/bin")

	if(NOT IMPL_NO_TARGET)
		nbl_install_exe_spec("${IMPL_TARGET}" "${_NBL_EXAMPLE_RUNTIME_RELATIVE_DIR}/bin" COMPONENT "${IMPL_COMPONENT}")
	endif()

	foreach(_NBL_EXAMPLE_RUNTIME_DIRECTORY IN LISTS IMPL_DIRECTORIES)
		if(IS_ABSOLUTE "${_NBL_EXAMPLE_RUNTIME_DIRECTORY}")
			message(FATAL_ERROR "nbl_install_example_runtime_component DIRECTORIES entries must be relative: ${_NBL_EXAMPLE_RUNTIME_DIRECTORY}")
		endif()
		install(DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/${_NBL_EXAMPLE_RUNTIME_DIRECTORY}"
			DESTINATION "${_NBL_EXAMPLE_RUNTIME_BIN_DESTINATION}"
	...
	foreach(_NBL_EXAMPLE_RUNTIME_FILE IN LISTS IMPL_FILES)
		if(IS_ABSOLUTE "${_NBL_EXAMPLE_RUNTIME_FILE}")
			message(FATAL_ERROR "nbl_install_example_runtime_component FILES entries must be relative: ${_NBL_EXAMPLE_RUNTIME_FILE}")
		endif()
		get_filename_component(_NBL_EXAMPLE_RUNTIME_FILE_DIRECTORY "${_NBL_EXAMPLE_RUNTIME_FILE}" DIRECTORY)
		install(FILES "${CMAKE_CURRENT_SOURCE_DIR}/${_NBL_EXAMPLE_RUNTIME_FILE}"
			DESTINATION "${_NBL_EXAMPLE_RUNTIME_BIN_DESTINATION}/${_NBL_EXAMPLE_RUNTIME_FILE_DIRECTORY}"
			COMPONENT "${IMPL_COMPONENT}"
		)
	endforeach()

	foreach(_NBL_EXAMPLE_RUNTIME_FILE IN LISTS IMPL_GENERATED_RUNTIME_FILES)
		if(IS_ABSOLUTE "${_NBL_EXAMPLE_RUNTIME_FILE}")
			message(FATAL_ERROR "nbl_install_example_runtime_component GENERATED_RUNTIME_FILES entries must be relative: ${_NBL_EXAMPLE_RUNTIME_FILE}")
		endif()
		get_filename_component(_NBL_EXAMPLE_RUNTIME_FILE_DIRECTORY "${_NBL_EXAMPLE_RUNTIME_FILE}" DIRECTORY)
		install(FILES "${CMAKE_CURRENT_BINARY_DIR}/bin/${_NBL_EXAMPLE_RUNTIME_FILE}"
			DESTINATION "${_NBL_EXAMPLE_RUNTIME_BIN_DESTINATION}/${_NBL_EXAMPLE_RUNTIME_FILE_DIRECTORY}"
			COMPONENT "${IMPL_COMPONENT}"
		)
	endforeach()

	get_target_property(_NBL_EXAMPLE_RUNTIME_EXE_INSTALL_DIR "${IMPL_TARGET}" NBL_PACKAGE_RUNTIME_EXE_DIR_PATH)
	get_target_property(_NBL_EXAMPLE_RUNTIME_NABLA_INSTALL_DIR Nabla NBL_PACKAGE_RUNTIME_DLL_DIR_PATH)
	get_property(_NBL_EXAMPLE_RUNTIME_DXC_INSTALL_DIR GLOBAL PROPERTY NBL_3RDPARTY_DXC_NS_PACKAGE_RUNTIME_DLL_DIR_PATH)
	if(NOT _NBL_EXAMPLE_RUNTIME_EXE_INSTALL_DIR OR NOT _NBL_EXAMPLE_RUNTIME_NABLA_INSTALL_DIR OR NOT _NBL_EXAMPLE_RUNTIME_DXC_INSTALL_DIR)
		message(FATAL_ERROR "nbl_install_example_runtime_component could not resolve runtime install directories for ${IMPL_TARGET}.")
	endif()

	cmake_path(RELATIVE_PATH _NBL_EXAMPLE_RUNTIME_EXE_INSTALL_DIR BASE_DIRECTORY "${_NBL_CPACK_PACKAGE_RELATIVE_ENTRY_}" OUTPUT_VARIABLE _NBL_EXAMPLE_RUNTIME_EXE_RELATIVE_DIR)
	cmake_path(RELATIVE_PATH _NBL_EXAMPLE_RUNTIME_NABLA_INSTALL_DIR BASE_DIRECTORY "${_NBL_CPACK_PACKAGE_RELATIVE_ENTRY_}" OUTPUT_VARIABLE _NBL_EXAMPLE_RUNTIME_NABLA_RELATIVE_DIR)
	cmake_path(RELATIVE_PATH _NBL_EXAMPLE_RUNTIME_DXC_INSTALL_DIR BASE_DIRECTORY "${_NBL_CPACK_PACKAGE_RELATIVE_ENTRY_}" OUTPUT_VARIABLE _NBL_EXAMPLE_RUNTIME_DXC_RELATIVE_DIR)

	set(_NBL_EXAMPLE_RUNTIME_MANIFEST_PATH "${CMAKE_CURRENT_BINARY_DIR}/nbl-example-runtime-manifests/${IMPL_COMPONENT}-$<CONFIG>.json")
	set(_NBL_EXAMPLE_RUNTIME_MANIFEST_CONTENT [=[
{
  "schema": "devsh.nabla.example-runtime.v1",
  "component": "@IMPL_COMPONENT@",
  "target": "@IMPL_TARGET@",
  "build_config": "$<CONFIG>",
  "executable": "@_NBL_EXAMPLE_RUNTIME_EXE_RELATIVE_DIR@/$<TARGET_FILE_NAME:@IMPL_TARGET@>",
  "bin": "@_NBL_EXAMPLE_RUNTIME_EXE_RELATIVE_DIR@",
  "nabla_runtime": "@_NBL_EXAMPLE_RUNTIME_NABLA_RELATIVE_DIR@",
  "dxc_runtime": "@_NBL_EXAMPLE_RUNTIME_DXC_RELATIVE_DIR@",
  "report_template": "@_NBL_EXAMPLE_RUNTIME_EXE_RELATIVE_DIR@/report"
}
]=])
	string(CONFIGURE "${_NBL_EXAMPLE_RUNTIME_MANIFEST_CONTENT}" _NBL_EXAMPLE_RUNTIME_MANIFEST_CONTENT @ONLY)
	file(GENERATE
		OUTPUT "${_NBL_EXAMPLE_RUNTIME_MANIFEST_PATH}"
		CONTENT "${_NBL_EXAMPLE_RUNTIME_MANIFEST_CONTENT}"
		TARGET "${IMPL_TARGET}"
	)
	install(FILES "${_NBL_EXAMPLE_RUNTIME_MANIFEST_PATH}"
		DESTINATION "${_NBL_CPACK_PACKAGE_RELATIVE_ENTRY_}"
		RENAME "${IMPL_COMPONENT}.json"
		COMPONENT "${IMPL_COMPONENT}"
	)

	get_property(_NBL_EXAMPLE_RUNTIME_COMPONENTS GLOBAL PROPERTY NBL_EXAMPLE_RUNTIME_COMPONENTS)
	if(NOT IMPL_COMPONENT IN_LIST _NBL_EXAMPLE_RUNTIME_COMPONENTS)
		set_property(GLOBAL APPEND PROPERTY NBL_EXAMPLE_RUNTIME_COMPONENTS "${IMPL_COMPONENT}")
	endif()

	if(IMPL_DISPLAY_NAME)
		set_property(GLOBAL PROPERTY "NBL_EXAMPLE_RUNTIME_COMPONENT_${IMPL_COMPONENT}_DISPLAY_NAME" "${IMPL_DISPLAY_NAME}")
	endif()
	if(IMPL_DESCRIPTION)
		set_property(GLOBAL PROPERTY "NBL_EXAMPLE_RUNTIME_COMPONENT_${IMPL_COMPONENT}_DESCRIPTION" "${IMPL_DESCRIPTION}")
	endif()
endfunction()
