	...
	macro(NBL_READ_VALIDATE_INSTALL_JSON_DEPENDENCIES _NBL_FIELD_TRAVERSAL_INCLUSIVE_ _NBL_JSON_DEPENDENCIES_LIST_CONTENT_)
		...
		if(NBL_ENABLE_PROJECT_JSON_CONFIG_VALIDATION)
			if(NOT "${_NBL_JSON_DEPENDENCIES_LIST_CONTENT_TYPE_}" STREQUAL ARRAY) # validate type
				message(FATAL_ERROR "Internal error while processing \"${_NBL_JSON_CONFIG_FILEPATH_}\". Wrong usage of NBL_READ_VALIDATE_INSTALL_JSON_DEPENDENCIES macro, input _NBL_JSON_DEPENDENCIES_LIST_CONTENT_ = \"${_NBL_JSON_DEPENDENCIES_LIST_CONTENT_}\" is not ARRAY type!")
			endif()
		endif()
			...
			foreach(_NBL_IDX_ RANGE ${_NBL_STOP_})
				...
				if(NBL_ENABLE_PROJECT_JSON_CONFIG_VALIDATION)
					if(NOT EXISTS "${_NBL_JSON_DEPENDENCY_FILEPATH_}") # validate if present
						message(FATAL_ERROR "Declared \"${_NBL_JSON_CONFIG_FILEPATH_}\"'s ${_NBL_FIELD_TRAVERSAL_INCLUSIVE_}[${_NBL_IDX_}] = \"${_NBL_JSON_DEPENDENCIES_LIST_ELEMENT_CONTENT_}\" doesn't exist! It's filepath is resolved to \"${_NBL_JSON_DEPENDENCY_FILEPATH_ABS_}\". Note that filepaths in json configs are resolved relative to them.")
					endif()
				endif()
		...
		if(_NBL_JSON_DATA_LEN_ GREATER_EQUAL 1)
			...
			foreach(_NBL_IDX_ RANGE ${_NBL_STOP_})
				...
				if(_NBL_JSON_COMMAND_LEN_ GREATER_EQUAL 1)
					...
					NBL_JSON_UPDATE_RELATIVE_REFERENCE_FILEPATH("${_NBL_JSON_TOP_CONFIG_CONTENT_}" data ${_NBL_IDX_} command)
