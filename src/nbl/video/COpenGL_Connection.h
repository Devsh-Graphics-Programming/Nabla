...
{
public:
    COpenGL_Connection(core::smart_refctd_ptr<system::ISystem>&& sys, SDebugCallback* dbgCb, system::logger_opt_smart_ptr&& logger): IAPIConnection(std::move(sys)), m_logger(std::move(logger))
    {
        ...
        if (m_egl.initialize())
            m_pdevice = PhysicalDeviceType::create(core::smart_refctd_ptr(m_system), core::smart_refctd_ptr(m_GLSLCompiler), &m_egl, dbgCb, system::logger_opt_smart_ptr(m_logger));
    }
