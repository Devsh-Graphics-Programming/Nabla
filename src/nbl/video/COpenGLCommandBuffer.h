...
#define _NBL_DEFINE_SCMD_SPEC(ECT) template<> \
struct SCmd<ECT> : SCmd_base<ECT>
...
#define _NBL_SCMD_TYPE_FOR_ECT(ECT) SCmd<impl::ECT>,
    struct SCommand
    {
        ...
        std::variant<
            NBL_FOREACH(_NBL_SCMD_TYPE_FOR_ECT, _NBL_COMMAND_TYPES_LIST) int>
            variant;

        template<impl::E_COMMAND_TYPE ECT>
        explicit SCommand(SCmd<ECT>&& cmd)
            : type(ECT), variant(std::move(cmd)) {}
...
public:
    ...
    bool bindDescriptorSets(
        ...
        const descriptor_set_t* const* const pDescriptorSets, const uint32_t dynamicOffsetCount = 0u, const uint32_t* dynamicOffsets = nullptr) override
    {
