...
public:
	static COpenGLPhysicalDevice* create(IAPIConnection* api, core::smart_refctd_ptr<system::ISystem>&& s, egl::CEGL&& _egl, COpenGLDebugCallback&& dbgCb)
	{
		...
		return new COpenGLPhysicalDevice(api,std::move(s),std::move(_egl),std::move(dbgCb),initRes.config,initRes.ctx,initRes.major,initRes.minor);
	}
...
protected:
	...
	{
		return core::make_smart_refctd_ptr<COpenGLLogicalDevice>(core::smart_refctd_ptr<IAPIConnection>(m_api),this,params,&m_egl,&m_glfeatures,m_config,m_gl_major,m_gl_minor);
	}

private:
    COpenGLPhysicalDevice(IAPIConnection* api, core::smart_refctd_ptr<system::ISystem>&& s, egl::CEGL&& _egl, COpenGLDebugCallback&& dbgCb, EGLConfig config, EGLContext ctx, EGLint major, EGLint minor)
