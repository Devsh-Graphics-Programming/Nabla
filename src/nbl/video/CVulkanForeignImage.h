...
public:
    CVulkanForeignImage(core::smart_refctd_ptr<ILogicalDevice>&& _vkdev,
        ...
        : CVulkanImage(std::move(_vkdev), std::move(_params), _vkimg, IDriverMemoryBacked::SDriverMemoryRequirements{/*TODO(Erfan):should any reqs be specified for foreign image???*/})
    {}
