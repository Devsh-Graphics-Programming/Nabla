...
public:
	static core::smart_refctd_ptr<COpenGLESPhysicalDevice> create(core::smart_refctd_ptr<io::IFileSystem>&& fs, core::smart_refctd_ptr<asset::IGLSLCompiler>&& glslc, const egl::CEGL* _egl)
	{
		...
		auto* pdev = new COpenGLESPhysicalDevice(std::move(fs), std::move(glslc), _egl, initRes.config, initRes.ctx, initRes.major, initRes.minor);
...
protected:
	...
	{
		return core::make_smart_refctd_ptr<COpenGLESLogicalDevice>(m_egl, &m_glfeatures, m_config, m_gl_major, m_gl_minor, params, core::smart_refctd_ptr(m_fs), core::smart_refctd_ptr(m_GLSLCompiler));
	}

private:
	COpenGLESPhysicalDevice(core::smart_refctd_ptr<io::IFileSystem>&& fs, core::smart_refctd_ptr<asset::IGLSLCompiler>&& glslc, const egl::CEGL* _egl, EGLConfig config, EGLContext ctx, EGLint major, EGLint minor) :
