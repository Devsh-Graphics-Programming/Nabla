...
IGPUDescriptorSet::IGPUDescriptorSet(core::smart_refctd_ptr<const IGPUDescriptorSetLayout>&& layout, core::smart_refctd_ptr<IDescriptorPool>&& pool, const uint32_t poolOffset, IDescriptorPool::SDescriptorOffsets&& offsets)
    : base_t(std::move(layout)), IBackendObject(std::move(core::smart_refctd_ptr<const ILogicalDevice>(pool->getOriginDevice()))), m_version(0ull), m_pool(std::move(pool)), m_poolOffset(poolOffset), m_descriptorStorageOffsets(std::move(offsets))
{
...
IGPUDescriptorSet::~IGPUDescriptorSet()
{
    if (!isZombie())
    {
        m_pool->deleteSetStorage(this);
        if (m_pool->m_creationParameters.flags.hasFlags(IDescriptorPool::ECF_FREE_DESCRIPTOR_SET_BIT))
            m_pool->m_descriptorSetAllocator.free_addr(m_poolOffset, 1);
    }
}
