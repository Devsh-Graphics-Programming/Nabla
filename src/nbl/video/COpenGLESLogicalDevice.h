...
public:
    COpenGLESLogicalDevice(const egl::CEGL* _egl, FeaturesType* _features, EGLConfig config, EGLint major, EGLint minor, const SCreationParams& params, core::smart_refctd_ptr<io::IFileSystem>&& fs, core::smart_refctd_ptr<asset::IGLSLCompiler>&& glslc) :
        base_t(_egl, EAT_OPENGL_ES, _features, config, major, minor, params, std::move(fs), std::move(glslc))
    {
