...
public:
        ...
        for (uint32_t i = 0u; i < params.queueParamsCount; ++i)
        {
            ...
            for (uint32_t j = 0u; j < qci.count; ++j)
            {
                ...
                (*m_queues)[ix] = new CThreadSafeGPUQueueAdapter(
                    new QueueType(core::smart_refctd_ptr<IOpenGL_LogicalDevice>(this), _egl,
                        m_glfeatures, ctxid, glctx.ctx, glctx.pbuffer, famIx, flags, priority,
                        static_cast<COpenGLDebugCallback*>(physicalDevice->getDebugCallback())),
    ...
    {
        return core::make_smart_refctd_ptr<IDescriptorPool>(core::smart_refctd_ptr<IOpenGL_LogicalDevice>(this));
    }
