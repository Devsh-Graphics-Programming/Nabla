...
bool IGPUCommandBuffer::begin(core::bitflag<E_USAGE> flags, const SInheritanceInfo* inheritanceInfo)
{
    ...
    if (inheritanceInfo)
    {
        ...
        if (!m_cmdpool->emplace<IGPUCommandPool::CBeginCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPURenderpass>(inheritanceInfo->renderpass.get()), core::smart_refctd_ptr<const IGPUFramebuffer>(inheritanceInfo->framebuffer.get())))
            return false;
    }
...
bool IGPUCommandBuffer::bindIndexBuffer(const buffer_t* buffer, size_t offset, asset::E_INDEX_TYPE indexType)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBindIndexBufferCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUBuffer>(buffer)))
        return false;
...
bool IGPUCommandBuffer::drawIndirect(const buffer_t* buffer, size_t offset, uint32_t drawCount, uint32_t stride)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CDrawIndirectCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUBuffer>(buffer)))
        return false;
...
bool IGPUCommandBuffer::drawIndexedIndirect(const buffer_t* buffer, size_t offset, uint32_t drawCount, uint32_t stride)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CDrawIndexedIndirectCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const buffer_t>(buffer)))
        return false;
...
bool IGPUCommandBuffer::drawIndirectCount(const buffer_t* buffer, size_t offset, const buffer_t* countBuffer, size_t countBufferOffset, uint32_t maxDrawCount, uint32_t stride)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CDrawIndirectCountCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const buffer_t>(buffer), core::smart_refctd_ptr<const buffer_t>(countBuffer)))
        return false;
...
bool IGPUCommandBuffer::drawIndexedIndirectCount(const buffer_t* buffer, size_t offset, const buffer_t* countBuffer, size_t countBufferOffset, uint32_t maxDrawCount, uint32_t stride)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CDrawIndexedIndirectCountCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const buffer_t>(buffer), core::smart_refctd_ptr<const buffer_t>(countBuffer)))
        return false;
...
bool IGPUCommandBuffer::beginRenderPass(const SRenderpassBeginInfo* pRenderPassBegin, asset::E_SUBPASS_CONTENTS content)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBeginRenderPassCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPURenderpass>(pRenderPassBegin->renderpass), core::smart_refctd_ptr<const IGPUFramebuffer>(pRenderPassBegin->framebuffer)))
        return false;
    ...
    uint32_t imageMemoryBarrierCount, const SImageMemoryBarrier* pImageMemoryBarriers)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CPipelineBarrierCmd>(m_segmentListHead, m_segmentListTail, bufferMemoryBarrierCount, bufferResources, imageMemoryBarrierCount, imageResources))
        return false;
    ...
    const descriptor_set_t* const* const pDescriptorSets, const uint32_t dynamicOffsetCount, const uint32_t* dynamicOffsets)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBindDescriptorSetsCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUPipelineLayout>(layout), descriptorSetCount, descriptorSets_refctd))
        return false;
...
bool IGPUCommandBuffer::bindComputePipeline(const compute_pipeline_t* pipeline)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBindComputePipelineCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUComputePipeline>(pipeline)))
        return false;
...
bool IGPUCommandBuffer::updateBuffer(buffer_t* dstBuffer, size_t dstOffset, size_t dataSize, const void* pData)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CUpdateBufferCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUBuffer>(dstBuffer)))
        return false;
...
bool IGPUCommandBuffer::buildAccelerationStructures(const core::SRange<video::IGPUAccelerationStructure::DeviceBuildGeometryInfo>& pInfos, video::IGPUAccelerationStructure::BuildRangeInfo* const* ppBuildRangeInfos)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBuildAccelerationStructuresCmd>(m_segmentListHead, m_segmentListTail, accelerationStructures.size(), accelerationStructures.data(), buffers.size(), buffers.data()))
        return false;
...
bool IGPUCommandBuffer::copyAccelerationStructure(const video::IGPUAccelerationStructure::CopyInfo& copyInfo)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CCopyAccelerationStructureCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUAccelerationStructure>(copyInfo.src), core::smart_refctd_ptr<const IGPUAccelerationStructure>(copyInfo.dst)))
        return false;
...
bool IGPUCommandBuffer::copyAccelerationStructureToMemory(const video::IGPUAccelerationStructure::DeviceCopyToMemoryInfo& copyInfo)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CCopyAccelerationStructureToOrFromMemoryCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUAccelerationStructure>(copyInfo.src), core::smart_refctd_ptr<const IGPUBuffer>(copyInfo.dst.buffer)))
        return false;
...
bool IGPUCommandBuffer::copyAccelerationStructureFromMemory(const video::IGPUAccelerationStructure::DeviceCopyFromMemoryInfo& copyInfo)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CCopyAccelerationStructureToOrFromMemoryCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUAccelerationStructure>(copyInfo.dst), core::smart_refctd_ptr<const IGPUBuffer>(copyInfo.src.buffer)))
        return false;
...
bool IGPUCommandBuffer::resetQueryPool(video::IQueryPool* queryPool, uint32_t firstQuery, uint32_t queryCount)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CResetQueryPoolCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IQueryPool>(queryPool)))
        return false;
...
bool IGPUCommandBuffer::writeTimestamp(asset::E_PIPELINE_STAGE_FLAGS pipelineStage, video::IQueryPool* queryPool, uint32_t query)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CWriteTimestampCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IQueryPool>(queryPool)))
        return false;
...
bool IGPUCommandBuffer::writeAccelerationStructureProperties(const core::SRange<video::IGPUAccelerationStructure>& pAccelerationStructures, video::IQueryPool::E_QUERY_TYPE queryType, video::IQueryPool* queryPool, uint32_t firstQuery)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CWriteAccelerationStructurePropertiesCmd>(m_segmentListHead, m_segmentListTail, queryPool, asCount, accelerationStructures))
        return false;
...
bool IGPUCommandBuffer::beginQuery(video::IQueryPool* queryPool, uint32_t query, core::bitflag<video::IQueryPool::E_QUERY_CONTROL_FLAGS> flags)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBeginQueryCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IQueryPool>(queryPool)))
        return false;
...
bool IGPUCommandBuffer::endQuery(video::IQueryPool* queryPool, uint32_t query)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CEndQueryCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IQueryPool>(queryPool)))
        return false;
...
bool IGPUCommandBuffer::copyQueryPoolResults(video::IQueryPool* queryPool, uint32_t firstQuery, uint32_t queryCount, buffer_t* dstBuffer, size_t dstOffset, size_t stride, core::bitflag<video::IQueryPool::E_QUERY_RESULTS_FLAGS> flags)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CCopyQueryPoolResultsCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IQueryPool>(queryPool), core::smart_refctd_ptr<const IGPUBuffer>(dstBuffer)))
        return false;
...
bool IGPUCommandBuffer::bindGraphicsPipeline(const graphics_pipeline_t* pipeline)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBindGraphicsPipelineCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUGraphicsPipeline>(pipeline)))
        return false;
...
bool IGPUCommandBuffer::pushConstants(const pipeline_layout_t* layout, core::bitflag<asset::IShader::E_SHADER_STAGE> stageFlags, uint32_t offset, uint32_t size, const void* pValues)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CPushConstantsCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUPipelineLayout>(layout)))
        return false;
...
bool IGPUCommandBuffer::clearColorImage(image_t* image, asset::IImage::E_LAYOUT imageLayout, const asset::SClearColorValue* pColor, uint32_t rangeCount, const asset::IImage::SSubresourceRange* pRanges)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CClearColorImageCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUImage>(image)))
        return false;
...
bool IGPUCommandBuffer::clearDepthStencilImage(image_t* image, asset::IImage::E_LAYOUT imageLayout, const asset::SClearDepthStencilValue* pDepthStencil, uint32_t rangeCount, const asset::IImage::SSubresourceRange* pRanges)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CClearDepthStencilImageCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUImage>(image)))
        return false;
...
bool IGPUCommandBuffer::fillBuffer(buffer_t* dstBuffer, size_t dstOffset, size_t size, uint32_t data)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CFillBufferCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUBuffer>(dstBuffer)))
        return false;
...
bool IGPUCommandBuffer::bindVertexBuffers(uint32_t firstBinding, uint32_t bindingCount, const buffer_t* const* const pBuffers, const size_t* pOffsets)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBindVertexBuffersCmd>(m_segmentListHead, m_segmentListTail, firstBinding, bindingCount, pBuffers))
        return false;
...
bool IGPUCommandBuffer::dispatchIndirect(const buffer_t* buffer, size_t offset)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CDispatchIndirectCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUBuffer>(buffer)))
        return false;
...
bool IGPUCommandBuffer::setEvent(event_t* _event, const SDependencyInfo& depInfo)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CSetEventCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUEvent>(_event)))
        return false;
...
bool IGPUCommandBuffer::resetEvent(event_t* _event, asset::E_PIPELINE_STAGE_FLAGS stageMask)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CResetEventCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUEvent>(_event)))
        return false;
...
bool IGPUCommandBuffer::waitEvents(uint32_t eventCount, event_t* const* const pEvents, const SDependencyInfo* depInfo)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CWaitEventsCmd>(m_segmentListHead, m_segmentListTail, depInfo->bufBarrierCount, buffers_raw, depInfo->imgBarrierCount, images_raw, eventCount, pEvents))
        return false;
...
bool IGPUCommandBuffer::copyBuffer(const buffer_t* srcBuffer, buffer_t* dstBuffer, uint32_t regionCount, const asset::SBufferCopy* pRegions)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CCopyBufferCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUBuffer>(srcBuffer), core::smart_refctd_ptr<const IGPUBuffer>(dstBuffer)))
        return false;
...
bool IGPUCommandBuffer::copyImage(const image_t* srcImage, asset::IImage::E_LAYOUT srcImageLayout, image_t* dstImage, asset::IImage::E_LAYOUT dstImageLayout, uint32_t regionCount, const asset::IImage::SImageCopy* pRegions)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CCopyImageCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUImage>(srcImage), core::smart_refctd_ptr<const IGPUImage>(dstImage)))
        return false;
...
bool IGPUCommandBuffer::copyBufferToImage(const buffer_t* srcBuffer, image_t* dstImage, asset::IImage::E_LAYOUT dstImageLayout, uint32_t regionCount, const asset::IImage::SBufferCopy* pRegions)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CCopyBufferToImageCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUBuffer>(srcBuffer), core::smart_refctd_ptr<const IGPUImage>(dstImage)))
        return false;
...
bool IGPUCommandBuffer::blitImage(const image_t* srcImage, asset::IImage::E_LAYOUT srcImageLayout, image_t* dstImage, asset::IImage::E_LAYOUT dstImageLayout, uint32_t regionCount, const asset::SImageBlit* pRegions, asset::ISampler::E_TEXTURE_FILTER filter)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CBlitImageCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUImage>(srcImage), core::smart_refctd_ptr<const IGPUImage>(dstImage)))
        return false;
...
bool IGPUCommandBuffer::copyImageToBuffer(const image_t* srcImage, asset::IImage::E_LAYOUT srcImageLayout, buffer_t* dstBuffer, uint32_t regionCount, const asset::IImage::SBufferCopy* pRegions)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CCopyImageToBufferCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUImage>(srcImage), core::smart_refctd_ptr<const IGPUBuffer>(dstBuffer)))
        return false;
...
bool IGPUCommandBuffer::resolveImage(const image_t* srcImage, asset::IImage::E_LAYOUT srcImageLayout, image_t* dstImage, asset::IImage::E_LAYOUT dstImageLayout, uint32_t regionCount, const asset::SImageResolve* pRegions)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CResolveImageCmd>(m_segmentListHead, m_segmentListTail, core::smart_refctd_ptr<const IGPUImage>(srcImage), core::smart_refctd_ptr<const IGPUImage>(dstImage)))
        return false;
...
bool IGPUCommandBuffer::executeCommands(uint32_t count, cmdbuf_t* const* const cmdbufs)
{
    ...
    if (!m_cmdpool->emplace<IGPUCommandPool::CExecuteCommandsCmd>(m_segmentListHead, m_segmentListTail, count, cmdbufs))
        return false;
