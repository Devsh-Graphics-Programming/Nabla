...
public:
    // signaled ctor
    COpenGLFence(core::smart_refctd_ptr<IOpenGL_LogicalDevice>&& dev, IOpenGL_FunctionTable* gl)
        : IGPUFence(core::smart_refctd_ptr<const ILogicalDevice>(dev), ECF_SIGNALED_BIT)
    {
    ...
    // un-signaled ctor
    explicit COpenGLFence(core::smart_refctd_ptr<IOpenGL_LogicalDevice>&& _dev)
        : IGPUFence(std::move(_dev), static_cast<E_CREATE_FLAGS>(0))
    {
    }
