...
public:
    CVulkanDescriptorSet(core::smart_refctd_ptr<ILogicalDevice>&& dev, core::smart_refctd_ptr<const IGPUDescriptorSetLayout>&& layout,
        core::smart_refctd_ptr<IDescriptorPool>&& pool, const uint32_t* descriptorStorageOffsets, VkDescriptorSet descriptorSet)
        : IGPUDescriptorSet(std::move(dev), std::move(layout), std::move(pool), descriptorStorageOffsets), m_descriptorSet(descriptorSet)
    {}
