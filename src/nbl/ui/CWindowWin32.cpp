...
LRESULT CALLBACK CWindowWin32::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case WM_CLOSE: {
            if(!eventCallback->onWindowClosed(window))
        ...
        case WM_SHOWWINDOW: {
            if(wParam = TRUE)
            {
                if(!eventCallback->onWindowShown(window))
                    shouldCallDefProc = false;
            }
            else
            {
                if(!eventCallback->onWindowHidden(window))
                    shouldCallDefProc = false;
            }
            break;
        }
        case WM_MOVE: {
            ...
            if(!eventCallback->onWindowMoved(window, newX, newY))
                shouldCallDefProc = false;
            break;
        }
        case WM_SIZE: {
            ...
            {
                case SIZE_MAXIMIZED:
                    if(!eventCallback->onWindowMaximized(window))
                        shouldCallDefProc = false;
                    break;
                case SIZE_MINIMIZED:
                    if(!eventCallback->onWindowMinimized(window))
                        shouldCallDefProc = false;
                    break;
            }
            break;
