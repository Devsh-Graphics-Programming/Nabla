...
CWindowXCB::CWindowXCB(core::smart_refctd_ptr<CWindowManagerXCB>&& winManager, SCreationParams&& params):
    ...
    m_handle.m_window = xcb.pxcb_generate_id(m_connection->getRawConnection());
    m_handle.m_connection = m_connection->getRawConnection();
    ...
    xcb_void_cookie_t xcbCheckResult = xcb.pxcb_create_window(
        m_connection->getRawConnection(), XCB_COPY_FROM_PARENT, m_handle.m_window, primaryScreen->root,
    ...
    m_connection->setMotifWmHints(m_handle.m_window, motifHints);
    ...
    xcb.pxcb_map_window(m_connection->getRawConnection(), m_handle.m_window);
...
bool CWindowXCB::setWindowSize_impl(uint32_t width, uint32_t height) {
    ...
    xcbIccm.pxcb_icccm_set_wm_normal_hints(m_connection->getRawConnection(), m_handle.m_window, &hints);
    return  true;
}
...
bool CWindowXCB::setWindowPosition_impl(int32_t x, int32_t y) {
    ...
    auto cookie = xcb.pxcb_configure_window_checked(m_connection->getRawConnection(), m_handle.m_window, XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y, values);
...
void CWindowXCB::setCaption(const std::string_view& caption) {
    ...
    xcb.pxcb_change_property(m_connection->getRawConnection(), XCB_PROP_MODE_REPLACE, m_handle.m_window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8, static_cast<uint32_t>(caption.size()), reinterpret_cast<const void* const>(caption.data()));
    xcb.pxcb_flush(m_connection->getRawConnection());
}
...
bool CWindowXCB::setWindowVisible_impl( bool visible) {
    ...
    if(visible) {
        ...
        xcb.pxcb_unmap_window(m_connection->getRawConnection(), m_handle.m_window);
        xcb.pxcb_flush(m_connection->getRawConnection());
    }
    return true;
}
