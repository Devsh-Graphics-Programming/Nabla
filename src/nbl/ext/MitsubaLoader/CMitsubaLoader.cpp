...
// @Crisspl doesn't OBJ also use derivative mapping??
// @Anastazluk will nneed this for glTF
namespace
{
...
SContext::tex_ass_type CMitsubaLoader::cacheTexture(SContext& ctx, uint32_t hierarchyLevel, const CElementTexture* tex)
{
	...
	switch (tex->type)
	{
		case CElementTexture::Type::BITMAP:
		{
				...
				if (!view)
				{
					asset::SAssetBundle imgBundle = interm_getAssetInHierarchy(m_assetMgr,tex->bitmap.filename.svalue,ctx.inner.params,hierarchyLevel,ctx.override_);
