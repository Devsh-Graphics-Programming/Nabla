...
protected:
    ...
    struct SContext
    {
        SContext(const SAssetLoadParams& _params, system::IFile* _mainFile, IAssetLoader::IAssetLoaderOverride* _override, uint32_t _hierarchyLevel)
            : loadContext(_params, _mainFile), loaderOverride(_override), hierarchyLevel(_hierarchyLevel) {}
        ~SContext() {}
...
private:
    ...
    core::smart_refctd_ptr<ICPURenderpassIndependentPipeline> getPipeline(
        const SContext& context, const E_PRIMITIVE_TOPOLOGY& primitiveType, const SVertexInputParams& vertexInputParams, const bool skinned, const bool hasUV, const bool hasColor) const
    {
