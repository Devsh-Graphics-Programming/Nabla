						...
						else
						{
							...
							core::smart_refctd_ptr<CGLTFPipelineMetadata> glTFPipelineMetadata;
							{
								if (ds3lAvailableFlag)
								{
									...
									glTFPipelineMetadata = core::make_smart_refctd_ptr<CGLTFPipelineMetadata>(materialParameters, core::smart_refctd_ptr(shaderInputsMetadata));
								}
							}
							...
							assetManager->setAssetMetadata(cpuPipeline.get(), std::move(glTFPipelineMetadata));
