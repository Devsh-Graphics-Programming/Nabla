...
hlsl::shapes::OBB<> CPolygonGeometryManipulator::calculateOBB(const VertexCollection& vertices)
{
  ...
  auto findImprovedObbAxesFromUpperAndLowerTetrasOfBaseTriangle = [](const VertexCollection& vertices,
    ...
    {
      hlsl::float32_t3 q0, q1;     // Top and bottom vertices for lower and upper tetra constructions
      hlsl::float32_t3 f0, f1, f2; // Edge vectors towards q0; 
      hlsl::float32_t3 g0, g1, g2; // Edge vectors towards q1; 
      ...
      {
        f0 = normalize(q0 - baseTriangle.vertices[0]);
        f1 = normalize(q0 - baseTriangle.vertices[1]);
        f2 = normalize(q0 - baseTriangle.vertices[2]);
      ...
      {
        g0 = normalize(q1 - baseTriangle.vertices[0]);
        g1 = normalize(q1 - baseTriangle.vertices[1]);
        g2 = normalize(q1 - baseTriangle.vertices[2]);
  ...
  const auto* minVert = extremals.vertices.minPtr();
  const auto* maxVert = extremals.vertices.maxPtr();
