...
static std::string encloseWithinExtraInclGuards(std::string&& _glslCode, uint32_t _maxInclusions, const char* _identifier)
{
    ...
    std::replace_if(
        defBase_.begin(), defBase_.end(), [](char c) -> bool { return !::isalpha(c) && !::isdigit(c); }, '_');
    ...
    return genDefs() +
        "\n"
        "#ifndef " +
        defBase_ + std::to_string(_maxInclusions) +
        "\n" +
        "#line 1 \"" + _identifier + "\"\n" +
        _glslCode +
        "\n"
...
public:
    Includer(const asset::IIncludeHandler* _inclhndlr, const system::ISystem* _fs, uint32_t _maxInclCnt)
        : m_inclHandler(_inclhndlr), m_system(_fs), m_maxInclCnt{_maxInclCnt} {}
