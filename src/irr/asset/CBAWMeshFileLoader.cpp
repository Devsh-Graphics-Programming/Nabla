        ...
        */
        ...
        switch (hdr.blobType)
        {
            case asset::Blob::EBT_FINAL_BONE_HIERARCHY:
            {
                ...
                fetchRawBlob(FINALBONEHIERARCHY_HIERARCHY_LVL);
                break;
            }
            /*case asset::Blob::EBT_SKINNED_MESH:
            {
                constexpr uint32_t SKINNED_MESH_HIERARCHY_LVL = 0; // what's it's hierarchy lvl?
                fetchRawBlob(SKINNED_MESH_HIERARCHY_LVL);
                auto skinnedMeshBlob = reinterpret_cast<asset::SkinnedMeshBlobV2*>(blob);
            ...
            {
                constexpr uint32_t MESH_HIERARCHY_LVL = 1; // what's it's hierarchy lvl?
                fetchRawBlob(MESH_HIERARCHY_LVL);
                auto meshBlob = reinterpret_cast<asset::MeshBlobV2*>(blob);
