...
asset::SAssetBundle CSTLMeshFileLoader::loadAsset(io::IReadFile* _file, const asset::IAssetLoader::SAssetLoadParams& _params, asset::IAssetLoader::IAssetLoaderOverride* _override, uint32_t _hierarchyLevel)
{
    ...
    const size_t vtxSize = hasColor ? (3 * sizeof(float) + 4 + 4) : (3 * sizeof(float) + 4);
	{
		...
		std::vector<std::tuple<uint16_t, E_FORMAT, uint32_t>> perIndexDataThatChanges{std::make_tuple(0, asset::EF_R32G32B32_SFLOAT, 0), std::make_tuple(3, asset::EF_A2B10G10R10_SNORM_PACK32, 12) };
		if (hasColor)
			perIndexDataThatChanges.push_back(std::make_tuple(1, asset::EF_B8G8R8A8_UNORM, 16));
