	...
	static std::string getDefinitions(const std::string& _path)
	{
		...
		auto tilePackingOffsetsStr = [&] {
			...
			for (uint32_t i = 0u; i < pg_sz_log2; ++i)
				offsets += "uvec2(" + std::to_string(tilePacking[i].x+tile_padding) + "," + std::to_string(tilePacking[i].x+tile_padding) + ")" + (i==(pg_sz_log2-1u) ? "" : ",");
			return offsets;
		};
...
#define ADDR_Y_MASK uint((0x1u<<(ADDR_LAYER_SHIFT-ADDR_Y_SHIFT))-1u))" +
			...
			"\n\nconst uvec2 packingOffsets[] = uvec2[PAGE_SZ_LOG2](" + tilePackingOffsetsStr() + ");" + 
			"\n#endif\n";
	}
